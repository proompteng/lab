...
fail_if_matches \
  "Agents v1 agent-message ingestion must use the typed Effect request program instead of nested ad hoc handler try/catch" \
  'let payload: Record<string, unknown>|parseJsonBody[(]request[)].*catch|await Effect[.]runPromise[(]ingestAgentMessagesEffect' \
  "${ROOT_DIR}/services/agents/src/server/v1/agent-messages.ts"

require_matches \
  "Agents v1 agent-message ingestion must expose the typed Effect request program and JSON body error" \
  'postAgentMessagesEffect|AgentMessagesJsonBodyError|parseJsonBodyEffect' \
  "${ROOT_DIR}/services/agents/src/server/v1/agent-messages.ts"
