...
const defaultOutputPath = resolve(repoRoot, 'argocd/applications/cloudflare/secrets.yaml')
const defaultOpCloudflaredTokenPath = 'op://infra/cloudflared/password'

const printUsage = (): never => {
  console.log(`Usage: bun run scripts/generate-cloudflare-sealed-secret.ts [output-path]

Reads the Cloudflare tunnel token from 1Password (override with CLOUDFLARED_TOKEN_OP_PATH),
seals it with kubeseal, and writes the SealedSecret manifest to
argocd/applications/cloudflare/secrets.yaml by default.
`)
  process.exit(0)
}
...
const opCloudflaredTokenPath = process.env.CLOUDFLARED_TOKEN_OP_PATH ?? defaultOpCloudflaredTokenPath
const sealedControllerName =
  process.env.CLOUDFLARE_SEALED_CONTROLLER_NAME ?? process.env.SEALED_SECRETS_CONTROLLER_NAME ?? 'sealed-secrets'
const sealedControllerNamespace =
  process.env.CLOUDFLARE_SEALED_CONTROLLER_NAMESPACE ??
  process.env.SEALED_SECRETS_CONTROLLER_NAMESPACE ??
  'sealed-secrets'

const secretName = 'cloudflared-token'
const secretNamespace = 'cloudflare'
...
const cloudflaredToken = await readSecret(opCloudflaredTokenPath)

if (!cloudflaredToken) {
  fatal(`Cloudflared tunnel token is empty. Check 1Password path: ${opCloudflaredTokenPath}`)
}

const tempDir = mkdtempSync(join(tmpdir(), 'cloudflared-token-'))
const tokenFilePath = join(tempDir, 'token.txt')

try {
  writeFileSync(tokenFilePath, cloudflaredToken, { mode: 0o600 })
} catch (error) {
...
console.log(`SealedSecret written to ${outputPath}. Commit and sync cloudflare to roll out the new token.`)
