...
on_exit() {
  local status=$?
  trap - EXIT
  if (( status != 0 )); then cat "/proof/$REHEARSAL_PHASE-engine.log" >&2 || true; fi
...
# Preserve the existing vnode allocation across changed upstream defaults.
export CASSANDRA_NUM_TOKENS=256
...
cqlsh 127.0.0.1 9042 -e 'SELECT JSON host_id,tokens FROM system.local;' |
  "$python_command" /scripts/verify-cassandra-ring.py >"/proof/$REHEARSAL_PHASE-ring.sha256"
if [[ "$REHEARSAL_PHASE" == target ]]; then
  cmp /proof/source-ring.sha256 /proof/target-ring.sha256
fi
