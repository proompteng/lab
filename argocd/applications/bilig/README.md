...
- The deployment uses `Recreate` and retains the `bilig-zero-replica` PVC so the cache is upgraded in place without creating a second replica against the same replica file. Startup and termination each allow ten minutes for replica initialization and graceful client draining, as recommended by the upstream self-hosting guide. The Deployment allows 30 minutes for rollout progress and Kargo allows 35 minutes for Argo synchronization, leaving scheduling and image-pull headroom beyond the shutdown and startup windows. Before rollout, require a Ready CNPG backup and record the existing replica integrity check, schema and table counts. After rollout, verify the new server version, upstream replication, query and mutation endpoints, and a client reconnect.
