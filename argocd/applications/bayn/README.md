...
The one-shot retirement Job, its dedicated Bayn egress policy, and the matching Restate admin-ingress exception are no
longer part of desired state. Any future `BaynLifecycle`, `BaynLifecycleBootstrap`, or `bayn-lifecycle-*` registration is
unexpected drift. Do not recreate the retired hook or mutate Restate registrations by hand; investigate the producer and
correct desired state through a reviewed GitOps change.
