...
2. `GrafWarmup.prime()` fetches the heavy singletons (`GraphService`, `WorkflowClient`, `ArgoWorkflowClient`, `MinioClient`, `AutoResearchLauncher`) concurrently. Doing this up front avoids work on the first FastAPI (REST) request and shifts the cost entirely into process startup.
