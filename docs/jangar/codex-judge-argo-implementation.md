...
| B) Notify wrapper                      | `docs/jangar/codex-judge-argo-implementation.md` | Retired             | Superseded by Agents runner status, log, artifact, and callback APIs.                                                                                                           | -                                                      | -                                                                                                          | The Jangar notify wrapper and `codex-implement` path are no longer the generic Codex execution boundary.                                                                                                          |
| C) Run-complete ingest + persistence   | `docs/jangar/codex-judge-argo-design.md`         | Retired             | Superseded by `services/agents/src/server/v1/agent-run-callbacks.ts`, runner events, and Agents Codex projection storage.                                                       | -                                                      | -                                                                                                          | Generic completion ingestion belongs to Agents; Jangar consumes domain projections.                                                                                                                               |
...
| L) Run history API + UI                | `docs/jangar/codex-judge-argo-design.md`         | Agents-owned        | `services/agents/src/server/v1/codex-runs.ts`, `packages/agent-contracts/src/codex-runs-client.ts`, Jangar PR detail route projection reads.                                    | -                                                      | -                                                                                                          | Standalone Jangar Codex run/search pages and compatibility APIs were removed; generic Codex projection reads belong to Agents.                                                                                    |
...
- Historical: the Argo wrapper posted notify payloads to Jangar.
- Current: runner status, logs, artifacts, callbacks, and Codex projection state are emitted through Agents-owned APIs.
...
The Jangar notify/run-complete callback endpoints described in this historical design are retired. Agents owns
AgentRun callback ingestion, runner event capture, and Codex projection updates.
...
- Keep Jangar out of generic completion ingestion; configure Agents callbacks and KafkaSources under
