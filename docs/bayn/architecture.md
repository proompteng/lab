...
A strategy rejection is terminal economic evidence, not an operational crash. A transient startup dependency failure
escapes the scoped runtime, closes HTTP and acquired clients, and lets the Deployment restart the process. A
deterministic contract or evidence failure enters `FAILED` and keeps HTTP available for diagnosis with readiness
closed.
