...
### Artifact identity gate and Image Factory cache caveat

Tags and version strings are discovery aids, not rollout identity. Select the exact accepted installer for the target,
verify that immutable image directly, and require both the installer and its extension input to appear in the release
receipt:

```bash
export TALOS_NODE='<target Talos API address>'
export RECEIPT='devices/galactic/extensions/kata/RELEASE-v4.1.0-talos-v1.13.9.md'
export SIGNING_IDENTITY='https://github.com/proompteng/lab/.github/workflows/kata-firecracker-extension.yaml@refs/heads/main'

case "$TALOS_NODE" in
  100.100.244.141)
    export INSTALLER_IMAGE='registry.ide-newton.ts.net/lab/talos-kata-runtimes@sha256:d489b3d7fc198d98dfbef8a8754933a5ce68f78920e4ce251f41a34594259def'
    export EXPECTED_KATA_EXTENSION='registry.ide-newton.ts.net/lab/talos-kata-runtimes@sha256:8d34965e669a53d3f7d7565d674fbe140acc0874422676601bb5638c705f8e8d'
    export EXPECTED_ARCH='amd64'
    ;;
  100.100.244.190)
    export INSTALLER_IMAGE='ghcr.io/proompteng/talos-kata-runtimes@sha256:fffaddf186ff39e4352b17fd032bac60aa518abac459346f43fde95586897db0'
    export EXPECTED_KATA_EXTENSION='ghcr.io/proompteng/talos-kata-runtimes@sha256:b7384435ad1393288e0235d8e467303348b252c2feb73973d309d07fee9afc44'
    export EXPECTED_ARCH='amd64'
    ;;
  100.100.244.142)
    export INSTALLER_IMAGE='ghcr.io/proompteng/talos-kata-runtimes@sha256:08a58afa7ca1ed0d02e23b9ff940edb37b131f0f1291392f2c00bdc9049dcfa2'
    export EXPECTED_KATA_EXTENSION='ghcr.io/proompteng/talos-kata-runtimes@sha256:b7384435ad1393288e0235d8e467303348b252c2feb73973d309d07fee9afc44'
    export EXPECTED_ARCH='arm64'
    ;;
  *) echo "no accepted Kata installer for $TALOS_NODE" >&2; exit 2 ;;
esac

test "$(crane digest "$INSTALLER_IMAGE")" = "${INSTALLER_IMAGE##*@}"
test "$(crane config "$INSTALLER_IMAGE" | jq -er .architecture)" = "$EXPECTED_ARCH"
cosign verify \
  --certificate-identity "$SIGNING_IDENTITY" \
  --certificate-oidc-issuer 'https://token.actions.githubusercontent.com' \
  "$INSTALLER_IMAGE" >/dev/null
rg -F "$INSTALLER_IMAGE" "$RECEIPT"
rg -F "$EXPECTED_KATA_EXTENSION" "$RECEIPT"
```

The NUC Image Factory exposes one catalog for a Talos version, and that live catalog currently resolves the accepted
r4 extension for Turin and Altra. It cannot simultaneously prove Ryzen's r5 direct installer. Run the separate catalog
verification in `devices/nuc/image-factory/README.md` only when the rollout will consume an installer generated by that
factory. For a reviewed same-schematic direct-install exception, the exact signed installer and its recorded workflow
receipt are the authority.
...
For an Omni/Image Factory phase, retain all of the following before allowing Omni to reboot the target:

1. the current `SchematicConfiguration` ID and ordered customization returned by
   `GET /schematics/<schematic-id>`;
2. the live factory catalog readback for the release that factory serves;
3. the generated `metal-installer` manifest digest for that exact schematic and Talos version; and
4. Image Factory build or registry evidence tying that installer to the selected extension digest.

For a same-schematic direct-install exception, retain the direct `crane` and `cosign` output above plus the recorded
main-branch workflow run that built the exact installer from `EXPECTED_KATA_EXTENSION`. Do not use a readback from a
different factory catalog as evidence for that direct installer.
