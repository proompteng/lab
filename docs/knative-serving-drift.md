...
2. Render the manifest locally, commit the desired-state change, and let Argo CD reconcile it:
   ```bash
   kubectl kustomize argocd/applications/<app>/overlays/cluster > /tmp/<app>-rendered.yaml
   ```
3. If a documented emergency _requires_ running `kn service update`, immediately
...
4. Double-check with `argocd app diff <app>` (or the web UI) before merging, then verify Argo CD reconciles the committed
   state.
