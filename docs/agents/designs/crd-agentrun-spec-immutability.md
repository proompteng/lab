...
- AgentRun schema: `services/agents/api/agents/v1alpha1/types.go` and `charts/agents/crds/agents.proompteng.ai_agentruns.yaml`.
- Controller reconciles AgentRuns and transitions phases/conditions:
  - `services/agents/src/server/agents-controller` (reconcileAgentRun, runtime submission, status updates).
