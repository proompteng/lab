...
- Template mappings (controllers):
  ...
  - default workload SA: `AGENTS_AGENT_RUNNER_SERVICE_ACCOUNT` from `controller.defaultWorkload.serviceAccountName`: `charts/agents/templates/deployment-controllers.yaml`
- Runtime uses these env vars while creating Jobs: `services/agents/src/server/agents-controller`.
...
| ----------------------------------------------- | ------------------------------------- | -------------------------------------------------------- |
| `controller.defaultWorkload.serviceAccountName` | `AGENTS_AGENT_RUNNER_SERVICE_ACCOUNT` | Primary default SA for agent-run Jobs.                   |
...
```bash
mise exec helm@3 -- helm template agents charts/agents -f argocd/applications/agents/values.yaml | rg -n \"AGENTS_AGENT_RUNNER_SERVICE_ACCOUNT|JANGAR_SCHEDULE_SERVICE_ACCOUNT\"
kubectl -n agents get deploy agents-controllers -o yaml | rg -n \"AGENTS_AGENT_RUNNER_SERVICE_ACCOUNT|JANGAR_SCHEDULE_SERVICE_ACCOUNT\"
```
