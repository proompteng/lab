...
- Generation: Agent primitives are generated from Go types (`go generate services/agents/api/agents`) and stamped
...
- Runtime checks: `services/agents/src/server/agents-controller` verifies required CRDs at startup by running
  `kubectl get <resource> -n <namespace>` and fails fast if they are missing or unauthorized. The required set
  expands when `AGENTS_CONTROLLER_VCS_PROVIDERS_ENABLED=true`.
...
- Update Go types under `services/agents/api/agents/v1alpha1` for agent primitives.
...
- Run `go generate services/agents/api/agents` to refresh CRDs and deepcopy outputs.
