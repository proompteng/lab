...
- The existing rule used an expression form (`has(self.parameters, 'prompt')`) that is rejected by Kubernetes CEL validation in our CI chart installation path.
...
- Option B: retain intent and switch to key-membership checks (`'prompt' in self.parameters`) (selected).
  - Pros: preserves validation intent and aligns with Kubernetes CEL map membership syntax.
...
- Choose Option B: replace invalid `has(self.parameters, 'prompt')` usage with map key-membership syntax in both chart CRD and all generated schema mirrors:
  - `!has(self.parameters) || !('prompt' in self.parameters)`
