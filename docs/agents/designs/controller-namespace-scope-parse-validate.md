...
Namespace scoping is a primary safety control for Agents controllers. The controllers accept a namespaces list via env vars (`AGENTS_CONTROLLER_NAMESPACES`, `AGENTS_PRIMITIVES_NAMESPACES`). Invalid JSON, explicit empty values, or ambiguous inputs can lead to unexpected reconciliation scope.
...
- Chart renders namespace lists into env vars:
  - `charts/agents/templates/deployment-controllers.yaml` sets `AGENTS_PRIMITIVES_NAMESPACES` and `AGENTS_CONTROLLER_NAMESPACES` via `agents.controllerNamespaces`.
- Runtime parsing helpers exist in `services/agents/src/server/agents-controller`:
...
```bash
kubectl -n agents get deploy agents-controllers -o yaml | rg -n \"AGENTS_CONTROLLER_NAMESPACES|AGENTS_PRIMITIVES_NAMESPACES\"
