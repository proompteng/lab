...
- Cluster (GitOps desired state): `JANGAR_AGENT_RUNNER_IMAGE` is set via `runner.image.*` in
  `argocd/applications/agents/values.yaml` (pin the image tag/digest there). `JANGAR_AGENT_RUNNER_JOB_TTL_SECONDS`
  is set to 600 by default, matching chart defaults unless overridden.
