...
- Go types: `services/agents/api/agents/v1alpha1/types.go` (ImplementationSpec types live in the same API package).
...
- Controller uses ImplementationSpecs during run submission:
  - `services/agents/src/server/agents-controller` (resolves ImplementationSpecRef / inline implementation).
