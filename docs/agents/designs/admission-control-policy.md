...
- Label policy: `AGENTS_CONTROLLER_LABELS_REQUIRED`, `..._ALLOWED`, `..._DENIED`.
- Image policy: `AGENTS_CONTROLLER_IMAGES_ALLOWED`, `..._DENIED`.
- Secret policy: `AGENTS_CONTROLLER_BLOCKED_SECRETS`.
- Agent-level policy:
...
- Enforcement lives in `services/agents/src/server/agents-controller` and sets `InvalidSpec` conditions when
  policy checks fail.
...
- `controller.admissionPolicy.labels.required` → `AGENTS_CONTROLLER_LABELS_REQUIRED`
- `controller.admissionPolicy.labels.allowed` → `AGENTS_CONTROLLER_LABELS_ALLOWED`
- `controller.admissionPolicy.labels.denied` → `AGENTS_CONTROLLER_LABELS_DENIED`
- `controller.admissionPolicy.images.allowed` → `AGENTS_CONTROLLER_IMAGES_ALLOWED`
- `controller.admissionPolicy.images.denied` → `AGENTS_CONTROLLER_IMAGES_DENIED`
- `controller.admissionPolicy.secrets.blocked` → `AGENTS_CONTROLLER_BLOCKED_SECRETS`
