...
- Go type preserves unknown fields:
  - `services/agents/api/agents/v1alpha1/types.go` → `AgentSpec.Config map[string]apiextensionsv1.JSON` with pruning preserve unknown fields.
...
- Controller consumes config dynamically (provider-specific):
  - Primary reconcile loop: `services/agents/src/server/agents-controller` (Agent/AgentRun reconciliation).
