...
- generated schedule-runner fire-time admission and proof checks are tied to the controller-level enforcement gate, so
  generated runner pods carry `JANGAR_SWARM_RUNTIME_ADMISSION_ENFORCEMENT` and the fire-time runner command checks it
  before any passport/proof lookup. Setting it to `false` returns schedules to advisory launch behavior without
  separately disabling runner admission checks.
