...
- degraded authority or runtime-kit evidence keeps the serving passport non-blocking as `degrade`, but compiles
  launch-capable swarm passports as `hold`, which the launcher gate treats the same as `block`.
