...
14. `14-dspy-jangar-openai-full-rollout-2026-02-27.md`

## Source-Verified Implementation Snapshot (2026-02-26 audit refresh)

- `01-tsfm-router-refinement-and-uncertainty.md`: `Partial`. Router/refinement/fallback paths and tests exist, and runtime uncertainty gate action handling is now wired in the trading scheduler.
- `02-conformal-uncertainty-and-regime-gates.md`: `Partial`. Uncertainty gate outputs (`pass/degrade/abstain/fail`) and promotion checks are wired, with runtime execution-path enforcement covered in scheduler logic and trading-pipeline tests.
...
- `10-crypto-market-data-pipeline-production-design-2026-02-22.md`: `Partial`. Desired-symbol fetch failure metrics and alerting are now wired (`ForwarderMetrics` + `TorghutWSDesiredSymbolsFetchFailing`); remaining work is full cutover/rollout validation.
- `11-multi-account-trading-architecture-and-rollout-2026-02-22.md`: `Partial` (feature-flagged). Account registry, per-account scheduler lanes, account-scoped idempotency/cursor constraints, and trade-updates v2 dual-read are merged; runtime keeps `TRADING_MULTI_ACCOUNT_ENABLED=false` by default.
- `12-dspy-framework-adoption-for-quant-llm-autonomous-trading-2026-02-25.md`: `Partial`. DSPy advisory/runtime scaffolding, compile/eval artifact schemas, Jangar-compatible AgentRun payload builder, and artifact persistence migration are merged; full promotion-governed runtime rollout remains pending.
- `13-fundamentals-news-codex-spark-agent-pipeline-2026-02-26.md`: `Partial`. Jangar market-context provider/ingest routes and Spark AgentRun contracts are wired; full-universe freshness/SLO stabilization remains ongoing.
- `14-dspy-jangar-openai-full-rollout-2026-02-27.md`: Production rollout design published. Legacy runtime LLM path removal and DSPy-only serving cutover are defined but not yet completed in code/runtime.
...
9. `14-dspy-jangar-openai-full-rollout-2026-02-27.md`
10. `09-fully-autonomous-quant-llm-torghut-novel-alpha-system.md`
