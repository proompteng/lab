...
This is a historical strategy-build archive index, not a current implementation dashboard. Validate every reference
...
## Historical Source-Verified Implementation Snapshot (2026-02-26 audit refresh)
...
- `02-conformal-uncertainty-and-regime-gates.md`: Implemented (strict). Runtime uncertainty and regime gates are evaluated with fail-closed defaults and strict action ranking in `services/torghut/app/trading/scheduler.py`; evidence in `services/torghut/tests/test_trading_pipeline.py` and `services/torghut/tests/test_metrics.py`.
- Signal continuity/freshness controls were recorded as implemented in production code paths (ingest reason
  classification, continuity alerting/recovery, emergency-stop integration, and continuity-aware live-promotion
  blocking).
- Profitability evidence/gate artifacts were recorded as implemented (gate6 + benchmark/evidence/validation outputs),
  and runtime profitability telemetry was exposed via `GET /trading/profitability/runtime` in
...
- `13-fundamentals-news-codex-spark-agent-pipeline-2026-02-26.md`: Implemented (partial). Jangar market-context provider/ingest routes and Spark AgentRun contracts are wired; full-universe freshness/SLO stabilization remains ongoing.
...
## Historical Runtime Profitability Surface (2026-02-26)
