...
| 7. Worker polling loops | Worker poll/complete/heartbeat modularisation. | [`bruke/src/worker.zig`](../bruke/src/worker.zig), [`tests/worker`](../tests/worker) | Worker loops implemented with unit tests per module. | ⚠️ Polling, completion, and heartbeats now run end-to-end via Zig; remaining work covers graceful drain telemetry and shutdown metrics. |
