...
test('sticky cache evicts least-recently-used entries when capacity exceeded', async () => {
  await Effect.runPromise(
    Effect.gen(function* () {
      ...
      yield* cache.upsert({ key: keyOne, determinismState: EMPTY_STATE, lastAccessed: Date.now() })
      yield* Effect.sleep('2 millis')
      yield* cache.upsert({ key: keyTwo, determinismState: EMPTY_STATE, lastAccessed: Date.now() })
      yield* Effect.sleep('2 millis')
      yield* cache.upsert({ key: keyThree, determinismState: EMPTY_STATE, lastAccessed: Date.now() })
...
test('sticky cache expires entries after ttl', async () => {
  await Effect.runPromise(
    Effect.gen(function* () {
      ...
      yield* cache.upsert({ key, determinismState: EMPTY_STATE, lastAccessed: Date.now() })
