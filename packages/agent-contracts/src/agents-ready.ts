...
const fallbackGenericStatus = (namespace: string, error: string | null): AgentsControlPlaneStatus => {
  ...
  const agentRunIngestion: AgentRunIngestionStatus = {
    namespace,
    status: 'unknown',
    message,
  ...
  const controllerWitness = {
    ...
    namespace,
    decision: 'hold_material' as const,
  ...
  const workflows = {
    ...
    window_minutes: 0,
    top_failure_reasons: [],
    data_confidence: 'unknown' as const,
    collection_errors: 1,
  ...
  return {
    ...
    controller_ingestion_settlement: buildControlPlaneControllerIngestionSettlement({
      now: new Date(generatedAt),
      namespace,
      servingReadiness: 'down',
      controllerWitness,
      agentRunIngestion,
      executionTrust: {
        status: 'unknown',
