...
const DEFAULT_TEMPLATE_PATH = resolve(repoRoot, 'devices/turin/manifests/tailscale-extension-service.template.yaml')
const DEFAULT_OUTPUT_PATH = resolve(repoRoot, 'devices/turin/manifests/tailscale-extension-service.yaml')
...
const parseArgs = () => {
  ...
  if (args.length > 1) {
    fatal('Usage: generate-turin-extension-service.ts [output-path]')
  }
...
const readAuthKey = async () => {
  ensureCli('op')
  const opPath = process.env.TURIN_TAILSCALE_AUTHKEY_OP_PATH ?? DEFAULT_OP_TAILSCALE_AUTH_PATH
...
const main = async () => {
  ...
  const templatePath = process.env.TURIN_TAILSCALE_TEMPLATE_PATH ?? DEFAULT_TEMPLATE_PATH
  ...
  console.log(`Apply with: talosctl patch machineconfig --patch @${outputPath}`)
}
