...
const DEFAULT_TEMPLATE_PATH = resolve(repoRoot, 'devices/ryzen/manifests/tailscale-extension-service.template.yaml')
const DEFAULT_OUTPUT_PATH = resolve(repoRoot, 'devices/ryzen/manifests/tailscale-extension-service.yaml')
...
const parseArgs = () => {
  ...
  if (args.length > 1) {
    fatal('Usage: generate-ryzen-extension-service.ts [output-path]')
  }
...
const readAuthKey = async () => {
  ensureCli('op')
  const opPath = process.env.RYZEN_TAILSCALE_AUTHKEY_OP_PATH ?? DEFAULT_OP_TAILSCALE_AUTH_PATH
...
const main = async () => {
  ...
  const templatePath = process.env.RYZEN_TAILSCALE_TEMPLATE_PATH ?? DEFAULT_TEMPLATE_PATH
