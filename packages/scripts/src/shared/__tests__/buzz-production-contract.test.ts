...
describe('Buzz production GitOps contract', () => {
  ...
  test('uses external stateful dependencies and Ceph while keeping unsafe features off', () => {
    ...
    expect(postgres).toContain('name: barman-cloud.cloudnative-pg.io')
    expect(postgres).toContain('enabled: true')
    expect(postgres).toContain('isWALArchiver: true')
    ...
    expect(redis.match(/argocd\.argoproj\.io\/sync-options: Prune=false,Delete=false/g)).toHaveLength(1)
