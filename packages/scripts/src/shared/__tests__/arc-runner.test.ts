...
describe('ARC Nix runner toolchain', () => {
  ...
  it('builds a custom actions runner image with pinned Nix CI tools preinstalled', () => {
    ...
    expect(arcRunnerImage).toContain('mkdir -p home/runner/.config/nix home/runner/tmpDir')
    expect(arcRunnerImage).toContain('chown -R 1001:1001 home/runner')
    expect(arcRunnerImage).toContain('chmod -R u+rwX,go+rX home/runner')
