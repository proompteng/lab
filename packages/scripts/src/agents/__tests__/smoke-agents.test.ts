...
describe('autonomous trader provider', () => {
  it('runs the market-open trader on its dedicated paper account', () => {
    ...
    expect(systemPromptText).toContain('--session-id "$sessionId" --record-tickets')
    expect(systemPromptText).toContain('Do not recreate helper tickets')
    expect(systemPromptText).not.toContain('call `autotrader_get_scorecard`')
    expect(greenSystemPromptText).toContain('--session-id "$sessionId" --record-tickets')
    expect(greenSystemPromptText).toContain('Do not recreate helper tickets')
