...
describe('autonomous trader provider', () => {
  ...
  it('runs the market-open trader on its dedicated paper account', () => {
    ...
    expect(systemPromptText).toContain('--session-id "$sessionId" --record-tickets --respect-account-gate')
    expect(systemPromptText).toContain('Read `sessionId` from `${AUTONOMOUS_TRADER_WORK_DIR}/session.json`')
    expect(systemPromptText).toContain('records gate status/risk')
    ...
    expect(greenSystemPromptText).toContain('--session-id "$sessionId" --record-tickets --respect-account-gate')
    expect(greenSystemPromptText).toContain('Read `sessionId` from `${AUTONOMOUS_TRADER_WORK_DIR}/session.json`')
    expect(greenSystemPromptText).toContain('records gate status/risk')
