...
describe('Tengri real guest acceptance helpers', () => {
  ...
  it('accepts only a lease bound to the authenticated cookie fingerprint and safe owned paths', () => {
    const authFingerprint = sha256Hex('tengri_session=primary')
    const lease = {
      version: 2,
    ...
    expect(validateLeaseRecord(lease, authFingerprint)).toMatchObject({
      agentId: lease.agentId,
      leaseId: lease.leaseId,
      agentCreatedAt: lease.agentCreatedAt,
      microvmUid: lease.microvmUid,
    })
    expect(() => validateLeaseRecord({ ...lease, version: 1 }, authFingerprint)).toThrow('invalid')
    expect(() => validateLeaseRecord(lease, sha256Hex('tengri_session=other'))).toThrow('another identity')
    expect(() => validateLeaseRecord({ ...lease, filePath: '/workspace/../other.txt' }, authFingerprint)).toThrow(
    ...
    const lease = validateLeaseRecord(
      {
        ...
        microvmUid: 'mvm-uid-acceptance',
        leaseId: '0123456789abcdef',
        authFingerprint: sha256Hex(options.authCookie),
        createdAt: '2026-09-07T00:00:00.000Z',
        firstRunId: 'run-123',
        terminalCreationId: 'tengri-acceptance-agent-acceptance-0123456789abcdef',
        filePath: '/workspace/.tengri-acceptance-0123456789abcdef.txt',
