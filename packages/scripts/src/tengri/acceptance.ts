...
export function validateLeaseRecord(value: unknown, authFingerprint: string): LeaseRecord {
  const record = asRecord(value)
  if (
    record?.version !== 2 ||
...
function readLease(path: string, authFingerprint: string): LeaseRecord | undefined {
  ...
  return validateLeaseRecord(parsed, authFingerprint)
}
...
async function ensureCanaryLease(state: ExecutionState, initialSnapshot: Snapshot): Promise<AgentSummary> {
  // Bind the lease to the configured session presented by the operator.  The BFF
  // may rotate Set-Cookie during the snapshot, so hashing the mutable jar would
  // make a durable lease unusable on a later run with the same configured secret.
  const fingerprint = sha256Hex(state.options.authCookie)
...
export async function cleanupState(state: ExecutionState): Promise<AcceptanceError | undefined> {
  ...
  if (state.runtime?.podName) resourceNames.add('pod/' + state.runtime.podName)
  if (state.runtime?.pvcName) resourceNames.add('pvc/' + state.runtime.pvcName)
...
export async function executeAcceptance(
  ...
  try {
    ...
    const primarySnapshot = await requireAuthenticatedSnapshot(state.primary, 'primary identity')
