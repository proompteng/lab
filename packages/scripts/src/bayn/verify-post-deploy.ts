...
export interface BaynPostDeploySnapshot {
  ...
  readonly restateApplication: unknown
  readonly restateOperatorApplication: unknown
  readonly restateOperatorCrdsApplication: unknown
...
const validateArgoApplicationHealth = (value: unknown, expectedName: string): JsonRecord => {
  ...
  equal(
    ...
    'Healthy',
    `${expectedName} application health`,
    'ARGO_NOT_CONVERGED',
    true,
  )
  return application
...
const validateArgo = (value: unknown): void => {
  const application = validateArgoApplicationHealth(value, 'bayn')
...
const validateRestateControlPlane = (snapshot: BaynPostDeploySnapshot): void => {
  validateArgoApplicationHealth(snapshot.restateApplication, 'restate')
  validateArgoApplicationHealth(snapshot.restateOperatorApplication, 'restate-operator')
  validateArgoApplicationHealth(snapshot.restateOperatorCrdsApplication, 'restate-operator-crds')
}
...
const validateDeployment = (value: unknown, expected: ExpectedBaynProduction): void => {
  ...
  if (deployment.status === undefined || deployment.status === null) {
    fail('WORKLOAD_NOT_CONVERGED', 'deployment.status is not projected yet', true)
  }
...
const validateStatus = (
  ...
  if (loop.lastPass === null || loop.lastPass === undefined) {
    fail('RUNTIME_NOT_READY', 'status.autonomousCycleLoop.lastPass is not available yet', true)
  }
  ...
  equal(cycle.zeroMutation, true, 'status.cycle.zeroMutation', 'PRODUCTION_CONTRACT_VIOLATION', false)
  ...
  equal(mutations.eventCount, 0, 'status.cycle.mutations.eventCount', 'PRODUCTION_CONTRACT_VIOLATION', false)
...
export const validateBaynPostDeploySnapshot = (
  ...
  validateRestateControlPlane(snapshot)
...
type HttpResult = { readonly status: number; readonly body: string }
export type ReadHttp = (url: string) => Promise<HttpResult>
...
export const readHttp: ReadHttp = async (url) => {
  try {
    ...
    return fail('READ_UNAVAILABLE', `failed to read ${url}`, true)
  }
}

const httpOutput = (result: HttpResult, label: string): string => {
  if (result.status < 200 || result.status >= 300) {
    return fail('READ_UNAVAILABLE', `${label} returned HTTP ${result.status}`, true)
  }
  return result.body
}

const jsonHttpOutput = (result: HttpResult, label: string): unknown => {
  const output = httpOutput(result, label)
  try {
    return JSON.parse(output) as unknown
  } catch {
    return fail('READ_UNAVAILABLE', `${label} returned invalid JSON`, true)
...
export const fetchBaynPostDeploySnapshot = async (
  run: RunCommand,
  get: ReadHttp = readHttp,
): Promise<BaynPostDeploySnapshot> => {
  const [
    application,
    restateApplication,
    restateOperatorApplication,
    restateOperatorCrdsApplication,
    deployment,
    executionController,
    readiness,
    status,
    ...
    run(['kubectl', 'get', 'application', 'restate', '-n', 'argocd', '-o', 'json']),
    run(['kubectl', 'get', 'application', 'restate-operator', '-n', 'argocd', '-o', 'json']),
    run(['kubectl', 'get', 'application', 'restate-operator-crds', '-n', 'argocd', '-o', 'json']),
    run(['kubectl', 'get', 'deployment', 'bayn', '-n', 'bayn', '-o', 'json']),
    run(['kubectl', 'get', 'restatedeployment', 'bayn-execution-controller', '-n', 'bayn', '-o', 'json']),
    get('http://bayn.bayn.svc.cluster.local/readyz'),
    get('http://bayn.bayn.svc.cluster.local/v1/status'),
    get('http://bayn.bayn.svc.cluster.local/metrics'),
  ])
  return {
    application: jsonOutput(application, 'Argo application'),
    restateApplication: jsonOutput(restateApplication, 'Restate Argo application'),
    restateOperatorApplication: jsonOutput(restateOperatorApplication, 'Restate Operator Argo application'),
    restateOperatorCrdsApplication: jsonOutput(
      restateOperatorCrdsApplication,
      'Restate Operator CRDs Argo application',
    ),
    ...
    readiness: jsonHttpOutput(readiness, 'Bayn readiness'),
    status: jsonHttpOutput(status, 'Bayn status'),
    metrics: httpOutput(metrics, 'Bayn metrics'),
  }
}
