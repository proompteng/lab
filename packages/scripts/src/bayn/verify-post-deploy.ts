...
export const parseExpectedBaynProduction = (
  ...
  equal(
    requiredEnv(deploymentEnv, 'BAYN_LIFECYCLE_OWNER', 'deployment'),
    'RESTATE',
    'deployment lifecycle owner',
    'INVALID_MANIFEST',
    false,
  )
  equal(
  ...
  equal(
    requiredEnv(controllerEnv, 'BAYN_LIFECYCLE_OWNER', 'executionController'),
    'RESTATE',
...
const validateStatus = (
  ...
  for (const [name, candidate] of Object.entries(record(status.dependencies, 'status.dependencies'))) {
...
export const fetchBaynPostDeploySnapshot = async (run: RunCommand): Promise<BaynPostDeploySnapshot> => {
  const [application, deployment, executionController, readiness, status, metrics] = await Promise.all([
    ...
    run(['kubectl', 'get', '--raw', '/api/v1/namespaces/bayn/services/bayn:80/proxy/readyz']),
    run(['kubectl', 'get', '--raw', '/api/v1/namespaces/bayn/services/bayn:80/proxy/v1/status']),
    run(['kubectl', 'get', '--raw', '/api/v1/namespaces/bayn/services/bayn:80/proxy/metrics']),
  ])
  return {
    ...
    readiness: jsonOutput(readiness, 'Bayn readiness'),
    status: jsonOutput(status, 'Bayn status'),
    metrics: requireCommand(metrics, 'Bayn metrics', true),
  }
}
