...
describe('Bayn lifecycle release manifests', () => {
  ...
  test('keeps the retired Restate endpoints canonical but inactive', () => {
    ...
    expect(readme).toContain('The retained failed pre-retirement Argo hook has also been garbage-collected')
    expect(readme).toContain('Future Bayn syncs therefore')
    expect(readme).toContain('cannot recreate that hook.')
