...
const healthyApplication = (name: string) => ({
  metadata: { name },
  status: {
...
const snapshot = () => ({
  ...
  restateApplication: healthyApplication('restate'),
  restateOperatorApplication: healthyApplication('restate-operator'),
  restateOperatorCrdsApplication: healthyApplication('restate-operator-crds'),
...
describe('Bayn production post-deploy contract', () => {
  ...
  test('fails while any required Restate control-plane application is degraded', () => {
    for (const field of [
      'restateApplication',
      ...
      const value = snapshot()
      ;(value[field] as any).status.health.status = 'Degraded'
      expect(
        failure(() => validateBaynPostDeploySnapshot(value, expected, now)),
        field,
      ).toMatchObject({
        code: 'ARGO_NOT_CONVERGED',
        retryable: true,
  ...
  test('retries while the Deployment status has not been projected yet', () => {
    const value = snapshot()
    delete (value.deployment as { status?: unknown }).status
    ...
    expect(result.message).toContain('deployment.status is not projected yet')
  })
  ...
  test('reads runtime evidence directly from the in-cluster Bayn service without Kubernetes service proxy access', async () => {
    const value = snapshot()
    const commandResults = new Map<string, unknown>([
      ['restatedeployment bayn-execution-controller', value.executionController],
      ['application bayn', value.application],
      ['application restate ', value.restateApplication],
      ['application restate-operator ', value.restateOperatorApplication],
      ['application restate-operator-crds ', value.restateOperatorCrdsApplication],
      ['deployment bayn', value.deployment],
    ])
    const commands: string[] = []
    const run = async (command: readonly string[]) => {
      const joined = command.join(' ')
      commands.push(joined)
      const match = [...commandResults.entries()].find(([needle]) => joined.includes(needle))
      return { stdout: JSON.stringify(match?.[1] ?? {}), stderr: '', exitCode: match === undefined ? 1 : 0 }
    }
    const urls: string[] = []
    const get = async (url: string) => {
      urls.push(url)
      if (url.endsWith('/readyz')) return { status: 200, body: JSON.stringify(value.readiness) }
      if (url.endsWith('/v1/status')) return { status: 200, body: JSON.stringify(value.status) }
      return { status: 200, body: value.metrics }
    }

    const observed = await fetchBaynPostDeploySnapshot(run, get)
    expect(() => validateBaynPostDeploySnapshot(observed, expected, now)).not.toThrow()
    expect(urls).toEqual([
      'http://bayn.bayn.svc.cluster.local/readyz',
      'http://bayn.bayn.svc.cluster.local/v1/status',
      'http://bayn.bayn.svc.cluster.local/metrics',
    ])
