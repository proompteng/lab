...
test('promotes verified main ancestry to an immutable GitOps branch', () => {
  ...
  expect(releaseWorkflow).toContain('git restore --source "$SOURCE_SHA" --')
  ...
  expect(releaseWorkflow.indexOf('git merge --no-edit -X theirs "$SOURCE_SHA"')).toBeLessThan(
    releaseWorkflow.indexOf('git restore --source "$SOURCE_SHA" --'),
  )
  expect(releaseWorkflow.indexOf('git restore --source "$SOURCE_SHA" --')).toBeLessThan(
