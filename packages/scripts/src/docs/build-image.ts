...
export const buildImage = async (options: BuildImageOptions = {}) => {
  ...
  const context = resolve(repoRoot, options.context ?? process.env.DOCS_BUILD_CONTEXT ?? '.')
  const dockerfile = resolve(repoRoot, options.dockerfile ?? process.env.DOCS_DOCKERFILE ?? 'apps/docs/Dockerfile')
  const platforms = options.platforms ??
    process.env.DOCS_IMAGE_PLATFORMS?.split(',').map((platform) => platform.trim()) ?? ['linux/arm64']
  const cacheRef = options.cacheRef ?? process.env.DOCS_BUILD_CACHE_REF ?? `${registry}/${repository}:buildcache`
