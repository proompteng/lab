...
it('requires every current node in the tailnet acceptance check', () => {
  ...
  expect(runbook).toContain('tailscale ping --c 1 --timeout 10s --until-direct=false "$node" >/dev/null')
  expect(runbook).toContain('printf \'required Tailscale node is offline or unreachable: %s\\n\' "$node" >&2')
  expect(runbook).toContain('exit 1')
  expect(runbook).not.toContain('tailnet_status="$(tailscale status)"')
