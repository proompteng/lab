...
export const main = async () => {
  ...
  const registry = process.env.KHOSHUT_IMAGE_REGISTRY ?? 'registry.ide-newton.ts.net'
  const repository = process.env.KHOSHUT_IMAGE_REPOSITORY ?? 'lab/khoshut'
  const defaultTag = execGit(['rev-parse', '--short', 'HEAD'])
  const tag = process.env.KHOSHUT_IMAGE_TAG ?? defaultTag
  ...
  const kustomizePath = resolve(repoRoot, process.env.KHOSHUT_KUSTOMIZE_PATH ?? 'argocd/applications/khoshut')
  await run('kubectl', ['apply', '-k', kustomizePath])
  const namespace = resolveKustomizeNamespace(kustomizePath)
  const deploymentName = process.env.KHOSHUT_K8S_DEPLOYMENT ?? 'khoshut'
  await run('kubectl', ['rollout', 'status', `deployment/${deploymentName}`, '-n', namespace])
}

if (import.meta.main) {
  main().catch((error) => fatal('Failed to build and deploy khoshut', error))
}
...
function updateManifests(options: ManifestUpdateOptions) {
  ...
  const kustomizationPath = resolve(repoRoot, 'argocd/applications/khoshut/kustomization.yaml')
  const kustomization = readFileSync(kustomizationPath, 'utf8')
  const updatedKustomization = kustomization.replace(
    /(-\s*name:\s+registry\.ide-newton\.ts\.net\/lab\/khoshut\s*\n\s*newTag:\s*).*/,
    (_, prefix) => `${prefix}"${tag}"`,
  )
  if (kustomization === updatedKustomization) {
    console.warn('Warning: khoshut kustomization was not updated; pattern may have changed.')
  } else {
    writeFileSync(kustomizationPath, updatedKustomization)
    console.log(`Updated ${kustomizationPath} with tag ${tag}`)
  }

  const deploymentPath = resolve(repoRoot, 'argocd/applications/khoshut/deployment.yaml')
  ...
  if (deployment === updatedDeployment) {
    console.warn('Warning: khoshut deployment rollout annotation was not updated; pattern may have changed.')
  } else {
...
function resolveKustomizeNamespace(kustomizePath: string) {
  const envNamespace = process.env.KHOSHUT_K8S_NAMESPACE?.trim()
  if (envNamespace) {
    return envNamespace
  }
  try {
