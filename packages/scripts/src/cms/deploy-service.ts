...
export const main = async () => {
  ...
  const registry = process.env.CMS_IMAGE_REGISTRY ?? 'registry.ide-newton.ts.net'
  const repository = process.env.CMS_IMAGE_REPOSITORY ?? 'lab/cms'
  const defaultTag = execGit(['rev-parse', '--short', 'HEAD'])
  const tag = process.env.CMS_IMAGE_TAG ?? defaultTag
...
if (import.meta.main) {
  main().catch((error) => fatal('Failed to build and deploy cms', error))
}
...
function updateManifests(options: ManifestUpdateOptions) {
  ...
  const kustomizationPath = resolve(repoRoot, 'argocd/applications/cms/kustomization.yaml')
  const kustomization = readFileSync(kustomizationPath, 'utf8')
  const updatedKustomization = kustomization.replace(
    /(-\s*name:\s+registry\.ide-newton\.ts\.net\/lab\/cms\s*\n\s*newTag:\s*).*/,
    (_, prefix) => `${prefix}"${tag}"`,
  )

  if (kustomization === updatedKustomization) {
    console.warn('Warning: CMS kustomization was not updated; pattern may have changed.')
  } else {
