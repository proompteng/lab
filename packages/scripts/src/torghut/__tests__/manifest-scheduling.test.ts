...
describe('Torghut manifest scheduling', () => {
  ...
  it('keeps options TA recoverable across transient Kafka source startup failures', () => {
    ...
    const job = getAtPath(spec, ['job'])
    expect(spec.restartNonce).toBeGreaterThanOrEqual(36)
    expect(job.upgradeMode).toBe('last-state')
    expect(flinkConfiguration['restart-strategy.type']).toBe('exponential-delay')
    expect(flinkConfiguration['restart-strategy.exponential-delay.initial-backoff']).toBe('10 s')
    expect(flinkConfiguration['restart-strategy.exponential-delay.max-backoff']).toBe('1 min')
    expect(flinkConfiguration['restart-strategy.exponential-delay.backoff-multiplier']).toBe('2.0')
    expect(flinkConfiguration['restart-strategy.exponential-delay.reset-backoff-threshold']).toBe('1 h')
    expect(flinkConfiguration['restart-strategy.exponential-delay.jitter-factor']).toBe('0.1')
    expect(Object.keys(flinkConfiguration).some((key) => key.startsWith('restart-strategy.fixed-delay.'))).toBe(false)
  })
