...
const parseArgs = (argv: string[]): CliOptions => {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === '--help' || arg === '-h') {
      console.log(`Usage: bun run packages/scripts/src/symphony/release-contract.ts <command> [options]
...
const assertValidContract = (contract: SymphonyReleaseContract) => {
...
export const writeReleaseContract = (path: string, contract: SymphonyReleaseContract) => {
  const normalized: SymphonyReleaseContract = {
    ...contract,
...
export const readReleaseContract = (path: string): SymphonyReleaseContract => {
  const source = readFileSync(resolveContractPath(path), 'utf8')
  const parsed = JSON.parse(source) as Partial<SymphonyReleaseContract>
  const contract: SymphonyReleaseContract = {
...
export const main = (cliOptions?: CliOptions) => {
  ...
  if (command === 'write') {
    writeReleaseContract(path, {
      sourceSha: parsed.sourceSha?.trim() ?? '',
      tag: parsed.tag?.trim() ?? '',
      digest: normalizeDigest(parsed.digest ?? ''),
...
if (import.meta.main) {
  try {
    ...
    fatal('Failed to process symphony release contract', error)
  }
}
