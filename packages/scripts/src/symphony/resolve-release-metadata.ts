...
const buildTriggerPathRegex =
  /^(services\/symphony\/|packages\/scripts\/src\/symphony\/|packages\/scripts\/src\/shared\/(?:cli|git)\.ts$|packages\/codex\/|packages\/otel\/|nix\/images\/symphony\.nix$|nix\/images\/openai-codex-cli\.nix$|nix\/images\/bun-workspace-service\.nix$|nix\/packages\.nix$|nix\/oci-release-contract\.sh$|flake\.lock$|package\.json$|bun\.lock$|tsconfig\.base\.json$|\.github\/workflows\/nix-oci-build-common\.yml$|\.github\/actions\/setup-nix-toolchain\/|\.github\/workflows\/symphony(?:-build-push|-release|-post-deploy-verify|-deploy-automerge|-ci)\.ya?ml$)/
