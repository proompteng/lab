...
| `src/crossplane/build-xpkgs.ts` | Builds (and optionally pushes) Crossplane configuration and function packages. | `crossplane` | `bun run packages/scripts/src/crossplane/build-xpkgs.ts --push --tag latest --tag-sha --function function-map-to-list=../crossplane-functions/map-to-list` |
