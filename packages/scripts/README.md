...
| `src/talos/build-firecracker-installer.ts` | Builds and pushes a Firecracker system extension plus a custom Talos installer image via `imager` (includes kata-containers, glibc, tailscale). | `docker`, `crane` | `bun run packages/scripts/src/talos/build-firecracker-installer.ts [tag-suffix | installer-tag extension-tag]` |
