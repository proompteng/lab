...
let terminalRequestStarted: (() => void) | undefined
let terminalRequestCancelled: (() => void) | undefined
...
beforeAll(async () => {
  ...
  server.addService(descriptor.proompteng.runtime.v1.MicroVMControlPlane.service, {
    ...
    createTerminal(call: grpc.ServerUnaryCall<Record<string, unknown>, Record<string, unknown>>) {
      terminalRequestStarted?.()
      call.on('cancelled', () => terminalRequestCancelled?.())
    },
  })
...
describe('Tengri gRPC BFF transport', () => {
  ...
  test('cancels terminal creation when the browser request disconnects', async () => {
    ...
    const request = createTerminal('github:42', 'agent-test', '/workspace', 120, 32, controller.signal)
    ...
    terminalRequestStarted = undefined
    terminalRequestCancelled = undefined
  })
})
