...
export async function createTerminal(
  ...
  return normalizeTerminal(
    await unary<RawRecord>('createTerminal', { agentId, cwd, columns, rows }, subject, 130_000, signal),
  )
}
...
async function unary<Response = RawRecord>(
  ...
  return new Promise((resolve, reject) => {
    ...
    call = method.call(
      ...
      (error, response) => {
        if (signal?.aborted) finish(() => reject(abortError(signal)))
        else if (error) finish(() => reject(mapGrpcError(error)))
        else finish(() => resolve(response as Response))
      },
    )
...
function abortError(signal?: AbortSignal) {
  ...
  const error = new Error('Tengri request was cancelled')
