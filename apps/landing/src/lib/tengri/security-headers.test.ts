...
describe('Tengri browser security headers', () => {
  test('keeps development-only execution and loopback allowances out of production', () => {
    ...
    expect(productionPolicy).toContain("'wasm-unsafe-eval'")
  ...
  test('limits connections to the gateway and frames to isolated preview origins', () => {
    ...
    expect(policy).toContain("frame-src 'self' https://tengri.proompteng.ai https://*.proompteng.ai")
    expect(policy).not.toContain('connect-src https://*.proompteng.ai')
