...
export function TerminalApp({
  ...
  const storageKey = terminalResumeStorageKey(agentId, windowId)
  ...
  useEffect(() => {
    ...
    let resetBeforeFreshSession = false
    const controller = new AbortController()
    const sessionController = new AbortController()
    const disposables: Array<{ dispose(): void }> = []
    const requestSignal = () => AbortSignal.any([controller.signal, AbortSignal.timeout(30_000)])
    const sessionRequestSignal = () => AbortSignal.any([sessionController.signal, AbortSignal.timeout(30_000)])
    ...
    const terminateSession = (terminalId: string) => {
      void runTengriAction({ action: 'terminate-terminal', agentId, terminalId }, { keepalive: true }).catch(
        () => undefined,
      )
    }

    const adoptSession = (candidate: TengriTerminalSession) => {
      if (terminalEnded) {
        clearStoredSession(candidate.id)
        terminateSession(candidate.id)
    ...
    const probeHeartbeat = () => {
      const target = socket
      if (document.visibilityState === 'hidden' || target?.readyState !== WebSocket.OPEN) return
      if (Date.now() - lastPongAt > 45_000) {
        target.close(4_000, 'Terminal heartbeat timed out')
        return
      }
    ...
    async function ensureSession(terminal: Terminal): Promise<TengriTerminalSession> {
      ...
      if (!session) {
        ...
        const created = await runTengriAction<TengriTerminalSession>(
        ...
        reconnectToken = ''
        lastSequence = 0
        if (resetBeforeFreshSession) {
          terminal.reset()
          terminal.clear()
          terminal.write('\r\n\x1b[33m[Previous terminal ended; started a new shell]\x1b[0m\r\n')
          resetBeforeFreshSession = false
        }
      }
      persist()
      return session
    ...
    const closeTerminal = () => {
      ...
      if (current) terminateSession(current.id)
    }
    registerWindowCloseHandler(windowId, closeTerminal)
    ...
    async function connect() {
      ...
      try {
        ...
        if (disposed || terminalEnded || controller.signal.aborted) return
    ...
    const handlePageShow = () => probeHeartbeat()
    const handleVisibilityChange = () => probeHeartbeat()
    ...
    return () => {
      if (!terminalEnded) persist()
      disposed = true
      reconnectNowRef.current = () => undefined
      if (!terminalEnded) sessionController.abort()
      controller.abort()
      registerWindowCloseHandler(windowId, null)
      ...
      socket?.close(1_000, 'Terminal view disconnected')
  ...
  }, [agentId, registerWindowCloseHandler, run, storageKey, windowId])
