...
export function AgentChat({ active, agentId }: { active: boolean; agentId: string }) {
  ...
  useEffect(() => {
    ...
    void runTengriAction<TengriCodexThread>({ action: 'resume-thread', agentId, threadId }, controller.signal)
      .then((thread) => {
        ...
        commitThread(
          ...
          setHistoryItems,
          setCurrentActiveTurnId,
          lastTurnLifecycleSequence.current <= resumeSequence,
        )
        setThreadReady(true)
        setError('')
      })
    ...
    return () => controller.abort()
  }, [account?.authenticated, active, agentId, setCurrentActiveTurnId, threadId, threadReady])
  ...
  const recoverThreadState = useCallback(async () => {
    ...
    try {
      ...
      commitThread(
        ...
        setHistoryItems,
        setCurrentActiveTurnId,
        lastTurnLifecycleSequence.current <= recoverySequence,
      )
      setThreadReady(true)
      setError('')
    } catch (cause) {
  ...
  useEffect(() => {
    ...
    return () => source.close()
  }, [account?.authenticated, active, agentId, recoverThreadState, setCurrentActiveTurnId])
  ...
  async function ensureThread() {
    ...
    setThreadReady(true)
    return { id: thread.id, activeTurnId: activeTurnIdRef.current }
  }
        ...
        <div className="mx-auto max-w-3xl space-y-3" role="log" aria-live="polite" aria-relevant="additions text">
          ...
          {renderedEvents.map(({ event, text }) => (
            <CodexEventCard
              ...
              approvalOptions={codexApprovalOptions(event)}
        ...
        {login ? (
          <div className="mt-5 rounded-xl bg-black/30 p-4">
            ...
            <button
              ...
              disabled={busy}
              onClick={onStart}
              className="mt-3 inline-flex items-center gap-1.5 text-xs text-white/48 outline-none hover:text-white/76 focus-visible:ring-2 focus-visible:ring-white/50 disabled:opacity-40"
            >
...
function commitThread(
  ...
  commitActiveTurn = true,
) {
  threadRef.current = thread.id
  setThreadId(thread.id)
  writeStoredThread(agentId, thread.id)
  const state = codexThreadStateFromThread(thread.rawJson)
  setHistoryItems(state.items)
  if (commitActiveTurn) setActiveTurnId(state.activeTurnId)
  return state
}
