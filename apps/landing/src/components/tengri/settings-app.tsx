...
export function SettingsApp({
  ...
  async function runLifecycle(action: 'delete-agent' | AgentLifecycleAction) {
    if (hasUnsavedChanges && action !== 'resume-agent') {
      setError('Save or close the pending Code tabs before sleeping or deleting this agent.')
      return
    }
    ...
    <div className="h-full overflow-auto bg-[#17191f] p-6 text-sm text-white/75">
      <div className="mx-auto max-w-xl space-y-5">
        ...
        >
          ...
          <div className="flex flex-wrap gap-2">
            {lifecycleAction ? (
              <button
                ...
                disabled={busyAction !== null || (hasUnsavedChanges && lifecycleAction !== 'resume-agent')}
            ...
            <button
              type="button"
              disabled={busyAction !== null || agent.phase === 'terminating' || hasUnsavedChanges}
              onClick={() => {
                setError('')
          ...
          {hasUnsavedChanges ? (
            <p role="status" className="mt-3 rounded-lg bg-amber-400/8 px-3 py-2 text-xs text-amber-100/75">
              Save or close the pending Code tabs to enable sleep and deletion.
            </p>
          ) : null}
          {error ? (
