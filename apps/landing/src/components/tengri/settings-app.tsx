...
export function SettingsApp({
  ...
  return (
    <div className="@container/settings relative flex h-full min-h-0 bg-[#1f2022] text-[13px] text-white/78">
      <div
        data-window-drag-region
        className="absolute top-0 left-0 h-[52px] w-[190px] touch-none select-none @max-[640px]/settings:w-[148px] @max-[480px]/settings:w-[96px]"
      />
      <aside className="flex w-[190px] shrink-0 flex-col border-r border-black/20 bg-gradient-to-b from-[#36363e] to-[#2b2b33] px-2.5 pt-[60px] pb-2.5 @max-[640px]/settings:w-[148px] @max-[480px]/settings:hidden">
        ...
        <nav aria-label="Settings sections" className="space-y-0.5">
          {settingsNavigation.map((item) => (
            <button
              ...
              className={`flex h-8 w-full items-center gap-2 rounded-md px-2 text-left text-[13px] transition-colors focus-visible:ring-2 focus-visible:ring-blue-400 focus-visible:outline-none ${selectedSection === item.id ? (active ? 'bg-[#2869ba] text-white' : 'bg-white/[0.12] text-white/85') : 'text-white/65 hover:bg-white/[0.07] hover:text-white/82'}`}
            >
      ...
      <div className="flex min-w-0 flex-1 flex-col bg-[#202123]">
        <div
          data-window-drag-region
          className="flex h-[52px] shrink-0 touch-none select-none items-center border-b border-black/25 bg-gradient-to-b from-[#343437] to-[#2d2d30] px-6 text-[14px] font-semibold text-white/90 @max-[480px]/settings:pl-[100px]"
        >
          System Settings
        </div>
        <div className="min-h-0 flex-1 overflow-auto">
          <div className="mx-auto w-full max-w-[660px] space-y-5 px-6 py-7 @max-[640px]/settings:px-4 @max-[640px]/settings:py-5">
            ...
            >
              <h2
                id={lifecycleHeadingId}
                className="mb-3 flex items-center gap-2 text-[11px] font-medium text-white/50"
              >
