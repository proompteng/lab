...
type SpotlightResult =
  ...
  | { id: string; kind: 'file'; label: string; detail: string; path: string; directory: boolean }
  | { id: string; kind: 'action'; label: string; detail: string; run: () => void }

export function spotlightResultOptionId(resultId: string) {
  return `spotlight-result-${encodeURIComponent(resultId)}`
}
...
export function Spotlight({
  ...
  const results = useMemo<SpotlightResult[]>(() => {
    ...
    return [...apps, ...actions, ...fileResults]
      ...
      .slice(0, 12)
  }, [files, onNewApp, onOpenApp, query, recentIds])
  ...
  useEffect(() => {
    setSelectedId((current) => (results.some((result) => result.id === current) ? current : results[0]?.id || ''))
  }, [results])
  ...
  useEffect(() => {
    resultRefs.current[selection]?.scrollIntoView({ block: 'nearest' })
  ...
  function activate(result: SpotlightResult | undefined) {
    ...
    if (result.kind === 'file') {
      if (result.directory) onOpenDirectory(result.path)
    ...
    >
      ...
      >
        <label className="flex h-16 items-center gap-3 border-b border-white/10 px-5">
          ...
          <input
            ref={inputRef}
            aria-activedescendant={selectedId ? spotlightResultOptionId(selectedId) : undefined}
            ...
            onKeyDown={(event) => {
              if (event.key === 'ArrowDown') {
                event.preventDefault()
                if (results.length) setSelectedId(results[Math.min(results.length - 1, selection + 1)]!.id)
              }
              if (event.key === 'ArrowUp') {
                event.preventDefault()
                if (results.length) setSelectedId(results[Math.max(0, selection - 1)]!.id)
              }
              if (event.key === 'Enter') {
                event.preventDefault()
          ...
          {searching && !results.length ? (
            ...
            results.map((result, index) => (
              <button
                ...
                type="button"
                id={spotlightResultOptionId(result.id)}
                role="option"
