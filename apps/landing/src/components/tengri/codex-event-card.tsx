...
type CodexEventCardProps = {
  approvalId?: string
  approvalOptions?: CodexApprovalDecision[]
...
export function CodexEventCard({
    ...
    return (
      ...
      >
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {approvalOptions.includes('approve-once') ? (
            <ApprovalButton
          ...
          {approvalOptions.includes('approve-session') ? (
            <ApprovalButton
              disabled={resolvingApproval}
              label="Approve for session"
              onClick={() => onResolveApproval('approve-session')}
              primary={!approvalOptions.includes('approve-once')}
            />
          ) : null}
          {approvalOptions.includes('deny') ? (
            <ApprovalButton disabled={resolvingApproval} label="Deny" onClick={() => onResolveApproval('deny')} />
          ) : null}
          {approvalOptions.length === 0 ? (
            <span className="text-xs text-amber-100/65" role="status">
              This approval requires a decision Tengri cannot safely submit yet.
            </span>
          ) : null}
          {resolvingApproval ? (
  ...
  if (kind === 'warning' || kind === 'error') {
    return (
      <article
        className={`mr-[8%] rounded-2xl border p-4 text-sm leading-6 ${
          kind === 'error'
...
function ApprovalButton({
  ...
  return (
    <button
      type="button"
      className={`rounded-lg px-3 py-1.5 text-xs font-medium outline-none focus-visible:ring-2 focus-visible:ring-white/60 disabled:opacity-40 ${
