...
export function DesktopGate({
  ...
  if (gate.kind === 'sign-in') return <SignInGate />
...
function SignInGate() {
  ...
  function signIn() {
    ...
    void tengriAuthClient.signIn
      ...
      .then((result) => {
        if (result.error) setError(result.error.message || 'Tengri could not start GitHub sign-in')
      })
      .catch((cause: unknown) =>
  ...
  const createAgent = handleSubmit(async ({ displayName }) => {
    try {
      ...
      setError('root.server', {
        message: cause instanceof Error ? cause.message : 'Agent could not be created',
      })
    }
  })
  ...
  return (
    <div className="fixed inset-0 z-[3000] grid place-items-center bg-black/16 p-5 backdrop-blur-sm">
      ...
      >
        ...
        >
          {isSubmitting ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <Play className="h-4 w-4 fill-current" />}
          Create Agent
        </button>
      </form>
    </div>
      ...
      >
        ...
        {actionLabel && onAction ? (
          <button
            ...
            disabled={actionBusy}
            onClick={onAction}
            className="mt-6 inline-flex items-center gap-2 rounded-xl bg-[#2574e8] px-4 py-2.5 text-sm font-semibold shadow-lg shadow-[#2574e8]/20 disabled:opacity-45"
          >
