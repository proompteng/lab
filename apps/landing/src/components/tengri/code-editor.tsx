...
export function CodeEditor({
  ...
  const pendingOpenPathsRef = useRef(new Set<string>())
  ...
  const savePath = useCallback(
    (targetPath: string, content: string, versionId: number) => {
      ...
      const operation = previous.then(async () => {
        ...
        try {
          await runTengriAction({ action: 'write-file', agentId, path: targetPath, content }, controller.signal)
  ...
  const loadPath = useCallback(
    async (targetPath: string, force = false) => {
      ...
      const initialModel = modelsRef.current.get(targetPath)
      const initialVersionId = initialModel?.getVersionId()

      requestsRef.current.get(targetPath)?.abort()
      const controller = new AbortController()
      requestsRef.current.set(targetPath, controller)
      ...
      try {
        ...
        if (
          force &&
          (currentTab?.dirty || (initialVersionId !== undefined && model && model.getVersionId() !== initialVersionId))
        ) {
        ...
        if (requestsRef.current.get(targetPath) === controller) requestsRef.current.delete(targetPath)
      }
    },
  ...
  useEffect(() => {
    ...
    pendingOpenPathsRef.current.add(requestPath)
    if (!editorReady) return
    if (!modelsRef.current.has(requestPath)) editorRef.current?.setModel(null)
    pendingOpenPathsRef.current.delete(requestPath)
    void loadPath(requestPath)
  }, [editorReady, loadPath, requestId, requestPath, updateTabs])

  useEffect(() => {
    if (!editorReady) return
    for (const path of pendingOpenPathsRef.current) {
  ...
  const migratePath = useCallback(
    async (previousPath: string, path: string) => {
      ...
      try {
        ...
        while (saveQueuesRef.current.has(previousPath)) {
          await saveQueuesRef.current.get(previousPath)
        }
        ...
        if (recentWriteTimers) {
          for (const timer of recentWriteTimers.values()) window.clearTimeout(timer)
        }
  ...
  useEffect(() => {
    ...
    const markConflict = (path: string, error: string) => {
      conflictedPathsRef.current.add(path)
      writeControllersRef.current.get(path)?.abort()
      const timer = saveTimersRef.current.get(path)
      if (timer) window.clearTimeout(timer)
      saveTimersRef.current.delete(path)
      patchTab(path, { dirty: true, state: 'error', error })
    }
    const verifyChange = (path: string) => {
      void runTengriAction<{ content: string }>({ action: 'read-file', agentId, path })
        .then((result) => {
          const current = tabsRef.current.find((tab) => tab.path === path)
          if (!current) return
          if (writeEchoesRef.current.matches(path, result.content)) return
          if (current.dirty) {
            markConflict(path, 'File changed outside Code while local edits were pending.')
          } else {
            void loadPath(path, true)
          }
        })
        .catch((cause: unknown) =>
          markConflict(path, cause instanceof Error ? cause.message : 'File change could not be verified'),
        )
    }
    const handleMessage = (directory: string, message: MessageEvent<string>) => {
      ...
      if (event.kind === 'reset') {
        for (const tab of tabsRef.current.filter((candidate) => codeParentDirectory(candidate.path) === directory)) {
          if (tab.dirty) {
            markConflict(tab.path, 'Filesystem state changed while local edits were pending.')
          } else {
      ...
      if (event.kind === 'renamed' && !event.previousPath) {
        markConflict(affected.path, 'File was renamed outside Code. Reopen it from Finder before saving.')
        return
      }
    ...
    const sources = directories.map((directory) => {
      ...
      const source = new EventSource(
        `/api/tengri/files/events?agentId=${encodeURIComponent(agentId)}&path=${encodeURIComponent(directory)}&after=${after}`,
      )
      source.onopen = () => {
  ...
  useEffect(() => onDirtyChangeRef.current?.(hasDirtyTabs), [hasDirtyTabs])
