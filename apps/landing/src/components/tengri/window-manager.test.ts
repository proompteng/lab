...
describe('Tengri window manager', () => {
  ...
  test('returns focus to Finder when every window is closed', () => {
    ...
    const hydrated = windowReducer(initialWindowState(viewport), { type: 'hydrate', state, viewport })
    expect(hydrated.activeApp).toBe('finder')
    expect(hydrated.activeWindowId).toBe('')
  })

  test('restores a minimized maximized window to its original bounds', () => {
    let state = initialWindowState(viewport)
    const id = state.activeWindowId
    const originalBounds = state.windows.find((window) => window.id === id)!.bounds
  ...
  test('enforces the same twenty-window cap during creation and hydration', () => {
    let state = initialWindowState(viewport)
