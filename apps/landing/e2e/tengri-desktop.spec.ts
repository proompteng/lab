...
const previewHost = `tengri-${'c'.repeat(24)}.proompteng.ai`
...
async function mockTengri(page: Page, options: MockOptions = {}) {
  ...
  const codexAuthenticated = options.codexAuthenticated ?? true
  const actions: Record<string, unknown>[] = []
  let codexLoginAttempts = 0
  ...
  await page.addInitScript(() => {
    ...
    class HealthyEventSource extends EventTarget {
      ...
      constructor(url: string | URL) {
        ...
        this.url = String(url)
        ;(
          window as typeof window & { __tengriEventSources?: Array<{ closed: boolean; url: string }> }
        ).__tengriEventSources?.push(this)
    ...
    Object.defineProperty(window, '__tengriEventSources', { configurable: true, value: [] })
  ...
  await page.routeWebSocket('wss://tengri.example.test/**', (socket) => {
    socket.onMessage(() => {})
  })
  await page.routeWebSocket(`wss://${previewHost}/**`, (socket) => {
    socket.onMessage((message) => {
      if (message === 'hmr:ping') socket.send('hmr:ready')
    })
  })
  await page.route(`https://${previewHost}/**`, async (route) => {
    await route.fulfill({
      contentType: 'text/html',
      body: `<!doctype html><title>Tengri preview</title><main>Live microVM preview</main><output>Connecting HMR</output><script>
        const output = document.querySelector('output');
        const socket = new WebSocket('wss://${previewHost}/hmr');
        socket.addEventListener('open', () => socket.send('hmr:ping'));
        socket.addEventListener('message', (event) => { output.textContent = event.data; });
      </script>`,
    })
  })
  await page.route('**/v1/preview/open', async (route) => {
    await route.fulfill({
      contentType: 'text/html',
      body: `<script>window.location.replace('https://${previewHost}/app?mode=dev')</script>`,
    })
  })

  await page.route('**/api/tengri', async (route) => {
    const request = route.request()
    const previewGatewayOrigin = `http://localhost:${new URL(request.url()).port || '80'}`
    ...
    switch (action.action) {
      ...
      case 'read-file':
        result = {
          ...
          contentType: String(action.path).endsWith('.md')
      ...
      case 'preview-session':
        result = {
          id: 'preview-1',
          launchUrl: `${previewGatewayOrigin}/v1/preview/open#${previewTicket}`,
          expiresAt: '2026-08-26T12:34:30.000Z',
        }
        break
      case 'codex-account':
        result = codexAuthenticated
          ? { authenticated: true, email: 'ada@example.test', plan: 'pro' }
          : { authenticated: false, email: '', plan: '' }
        break
      case 'codex-login':
        codexLoginAttempts += 1
        if (options.failCodexLoginRestart && codexLoginAttempts > 1) {
          await route.fulfill({
            status: 503,
            contentType: 'application/json',
            body: JSON.stringify({ error: 'The previous device login could not be cancelled' }),
          })
          return
        }
        result = {
          ...
          userCode: 'TENG-RI01',
          expiresAt: '2099-08-26T13:00:00.000Z',
        }
        break
      case 'create-thread':
      ...
      case 'create-terminal':
        if (options.terminalCreateDelayMs) {
          await new Promise((resolve) => setTimeout(resolve, options.terminalCreateDelayMs))
        }
        result = {
          id: 'terminal-1',
          cwd: '/workspace',
      ...
      case 'terminal-ticket':
        result = {
          ...
          websocketUrl: 'wss://tengri.example.test/v1/terminal/ws',
          expiresAt: '2026-08-26T12:34:30.000Z',
        }
        break
      case 'resume-thread':
        ...
        result = { id: action.threadId, rawJson: options.resumeThreadRawJson ?? '{"thread":{"turns":[]}}' }
        break
      case 'send-turn':
...
function activeFileWatchCount(page: Page) {
  return page.evaluate(
    () =>
      (
        window as typeof window & { __tengriEventSources?: Array<{ closed: boolean; url: string }> }
      ).__tengriEventSources?.filter((source) => !source.closed && source.url.includes('/api/tengri/files/events'))
        .length ?? 0,
  )
}
...
function emitCodexEvent(page: Page, event: Record<string, unknown>) {
  return page.evaluate((payload) => {
    ...
    ).__tengriEventSources?.find((candidate) => candidate.url.includes('/api/tengri/events?'))
...
test('supports Dock-only launching, Spotlight, menus, Finder Quick Look, and window controls', async ({ page }) => {
  ...
  await expect.poll(() => activeFileWatchCount(page)).toBe(1)
  ...
  await expect(spotlight.getByRole('combobox')).toHaveAttribute(
    'aria-activedescendant',
    'spotlight-result-app%3Asettings',
  )
  ...
  await dock.getByRole('button', { name: 'Open Finder' }).click()
  await expect(finder).toBeVisible()
  await expect(finder).toHaveCSS('pointer-events', 'auto')
...
test('terminates a PTY that finishes creating after its window closes', async ({ page }) => {
  const mock = await mockTengri(page, { terminalCreateDelayMs: 250 })
  await page.goto('/')
  ...
  await expect
    .poll(() =>
      mock.actions.some((action) => action.action === 'terminate-terminal' && action.terminalId === 'terminal-1'),
    )
    .toBe(true)
})

test('delivers a Finder file-open request only to its selected Code window', async ({ page }) => {
  await mockTengri(page)
  await page.goto('/')
  ...
  const codeWindows = page.locator('section[aria-label="Code window"]')
  await expect(codeWindows).toHaveCount(1)
  await expect(codeWindows.first().getByRole('tab', { name: /README\.md/ })).toBeVisible()
  await page.keyboard.press('Meta+n')
  await expect(codeWindows).toHaveCount(2)
  await expect(codeWindows.last().getByRole('tab', { name: /README\.md/ })).toHaveCount(0)
  await expect(codeWindows.first().getByRole('tab', { name: /README\.md/ })).toBeVisible()
})

test('reports the desktop window cap instead of losing a Finder file-open request', async ({ page }) => {
  await mockTengri(page)
  await page.goto('/')

  const windows = page.locator('section[aria-label$=" window"]')
  await expect(windows).toHaveCount(2)
  for (let index = 0; index < 18; index += 1) {
    await page.keyboard.press('Meta+n')
    await expect(windows).toHaveCount(index + 3)
  }
  ...
  await expect(
    page.getByRole('status').filter({ hasText: 'Close a window before opening this file in Code.' }),
  ).toBeVisible()
  await expect(page.getByRole('region', { name: 'Code window' })).toHaveCount(0)
})
...
test('persists real Finder changes into Code and exposes a localhost preview from Chrome', async ({ page }) => {
  ...
  await dock.getByRole('button', { name: 'Open Finder' }).click()
  await expect.poll(() => activeFileWatchCount(page)).toBeGreaterThanOrEqual(2)
  await finder.getByRole('button', { name: /README\.md/ }).click()
  await finder.getByRole('button', { name: 'Quick Look' }).click()
  ...
  await expect(chrome.getByTitle('localhost:4321')).toBeVisible()
  const preview = chrome.getByTitle('localhost:4321').contentFrame()
  await expect(preview.getByText('Live microVM preview')).toBeVisible()
  await expect(preview.getByText('hmr:ready')).toBeVisible()
  await expect
    .poll(() =>
      mock.actions.some(
...
test('sends a real agent turn and executes sleep, resume, and confirmed deletion', async ({ page }) => {
  ...
  const sleeping = page.getByRole('dialog', { name: 'Your agent is sleeping' })
...
test('keeps the active Codex device code visible when a restart fails', async ({ page }) => {
  await mockTengri(page, { codexAuthenticated: false, failCodexLoginRestart: true })
  await page.goto('/')

  await page.getByRole('button', { name: 'Start device login' }).click()
  await expect(page.getByText('TENG-RI01')).toBeVisible()
  await page.getByRole('button', { name: 'Start a new code' }).click()
  await expect(page.getByText('The previous device login could not be cancelled', { exact: true })).toBeVisible()
  await expect(page.getByText('TENG-RI01')).toBeVisible()
})
...
test('supports desktop window shortcuts, independent windows, drag, and eight-edge resize behavior', async ({
  ...
  const chromeWindows = page.locator('section[aria-label="Chrome window"]')
  ...
  const beforeResize = await frontmost.boundingBox()
  const resizeHandle = frontmost.locator('.cursor-se-resize')
  const resizeBounds = await resizeHandle.boundingBox()
  expect(resizeBounds).not.toBeNull()
  await page.mouse.move(resizeBounds!.x + 2, resizeBounds!.y + 2)
  await page.mouse.down()
  await page.mouse.move(resizeBounds!.x + 82, resizeBounds!.y + 62)
  await page.mouse.up()
  await expect.poll(async () => (await frontmost.boundingBox())?.width).toBeGreaterThan(beforeResize!.width + 55)

  await page.keyboard.press('Meta+Backquote')
  await page.keyboard.press('Meta+m')
  await expect(chromeWindows.first()).toHaveCSS('pointer-events', 'none')
...
test('renders truthful booting, sleeping, and failed lifecycle states', async ({ page }) => {
  ...
  await page.goto('/')
  await expect(page.getByRole('dialog', { name: 'Your agent is sleeping' })).toBeVisible()
  ...
  await page.reload()
  const failed = page.getByRole('alertdialog', { name: 'Agent could not start' })
...
test('matches the Tahoe desktop at required production viewports', async ({ page }) => {
  ...
  const developmentTools = page.getByRole('button', { name: 'Open Next.js Dev Tools' })
  if (await developmentTools.isVisible()) {
