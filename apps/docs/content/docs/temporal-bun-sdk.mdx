...
If you are evaluating the SDK for the first time, use this order:

- **Install and scaffold** if you want a working project quickly.
- **Configure your Temporal connection** if you need local, self-hosted, or Temporal Cloud settings.
- **Temporal Cloud and TLS** if you need API key auth, custom CA bundles, or mTLS:
  [Cloud and TLS guide](/docs/temporal-bun-sdk-cloud-tls).
- **Comparison guide** if you are deciding between Bun and the official Node.js path:
  [Bun SDK vs official TypeScript SDK](/docs/temporal-bun-sdk-comparison).
- **Run a worker** and **Start and manage workflows from Bun** for the core APIs.
- **Replay workflow histories** and **CLI quick reference** for operations and debugging.
- **Architecture overview** only if you need the runtime model.
...
Generate a standalone Bun worker project:
