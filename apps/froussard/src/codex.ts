...
const buildImplementationPrompt = ({
  ...
  return [
    ...
    `- Keep the progress comment anchored by ${PROGRESS_COMMENT_MARKER} current using the repository issue/comment APIs available in the runner.`,
    ...
    '- Do not request Codex review automatically. Do not post `@codex review` or `codex:review-request` comments.',
    '- Large PRs use the same gate as small PRs unless a human explicitly asks for review.',
    '- You are permitted to merge the PR once CI is green, merge conflicts are resolved, and all existing review comments are resolved.',
    ...
    '',
    'AgentRun status and artifacts:',
    '- The Agents app-server runner streams turn events and captures final status, logs, and declared artifacts.',
    '- Write durable outputs to declared artifact paths when the task requires them.',
    '- Do not depend on Jangar-local helper scripts or legacy progress publishers.',
    '',
    'Final reply (concise):',
    '- Summary (2–4 bullets)',
