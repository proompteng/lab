...
- In-cluster callbacks/auth: use projected service-account tokens only when the target validates Kubernetes service-account credentials. Torghut state-changing APIs require `TORGHUT_COMMAND_API_TOKEN` and must not use projected service-account tokens.
