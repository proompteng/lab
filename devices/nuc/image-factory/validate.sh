...
for command in docker ip openssl rg; do
...
ip -4 -brief address | rg -q "(^|[[:space:]])${bind_address}/25([[:space:]]|$)" || die "$bind_address/25 is not configured on this host"
...
rg -q '^\s+extensionManifest: proompteng/talos-extensions$' "$image_factory_dir/config.yaml"
rg -q '^\s+externalURL: http://100\.100\.244\.148:8080/$' "$image_factory_dir/config.yaml"
if rg -q '^\s+disabled: true$' "$image_factory_dir/config.yaml"; then
