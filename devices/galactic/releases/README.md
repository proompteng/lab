...
by upgrading one node at a time, in Ryzen, Turin, Altra order.
...
3. Export the live cluster through Omni 1.11. Move the imported `machine.install.disk` selections into each template
   Machine's `install.disk` field, preserving the exact verified boot disks. Omni 1.11 only automatically migrates its
...
4. Set Talos to `v1.14.0` through the committed Omni template, with rolling `maxParallelism: 1` and per-machine locks
   to release one target at a time. Preserve the existing CRI configuration needed for Kata blockfile snapshots. Do not
...
5. For each node, save an etcd snapshot from a peer, verify the installer receipt, drain, then let Omni perform the
   ...
   GPU, and QEMU/Cloud Hypervisor/Firecracker/Dragonball canaries before releasing the next node. Retain the existing
...
6. After all nodes pass, commit and sync the separate Kubernetes `v1.37.0` change through Omni. Verify all three
   ...
   separately. Remove temporary locks and maintenance flags and delete secret-filled local exports.
