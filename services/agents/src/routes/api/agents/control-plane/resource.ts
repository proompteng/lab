...
import { errorResponse, okResponse, parseJsonBody, requireIdempotencyKey } from '../../../../server/http'
...
export const getPrimitiveResource = async (
  ...
  try {
    ...
    if (resolveControlPlaneCacheReadConfig(process.env).enabled && cacheKinds.has(resolved.kind)) {
      ...
      try {
        ...
        if (cached) {
          ...
          console.warn('[agents][control-plane-cache] stale resource cache hit, falling back to live read', {
            kind: resolved.kind,
            namespace,
            name,
...
export const deletePrimitiveResource = async (request: Request, deps: { kubeClient?: KubernetesClient } = {}) => {
  const url = new URL(request.url)
  const kindParam = url.searchParams.get('kind')
  const name = asString(url.searchParams.get('name'))
  const resolved = resolvePrimitiveKind(kindParam)
