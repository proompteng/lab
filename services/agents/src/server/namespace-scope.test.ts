...
import { assertClusterScopedForWildcard, parseNamespaceScopeEnv } from './namespace-scope'
...
afterEach(() => {
  if (original == null) {
    delete process.env.AGENTS_RBAC_CLUSTER_SCOPED
  } else {
...
describe('namespace scope', () => {
  it('throws when wildcard namespaces are used without cluster RBAC', () => {
    ...
    expect(() => assertClusterScopedForWildcard(['*'], 'agents controller')).toThrow(
      "[agents] agents controller namespaces '*' require rbac.clusterScoped=true",
    )
  })

  it('allows wildcard namespaces when cluster RBAC is enabled', () => {
    process.env.AGENTS_RBAC_CLUSTER_SCOPED = 'true'
