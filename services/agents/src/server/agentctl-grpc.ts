...
const loadAgentctlPackage = (): AgentctlPackage => {
  ...
  if (!protoPath) {
    throw new Error('agentctl proto not found; set AGENTS_GRPC_PROTO_PATH or copy proto to runtime assets')
  }
...
const getGrpcLogHelper = () => {
  if (logState.__agentsGrpcLogHelper) return logState.__agentsGrpcLogHelper
  const kubeConfig = new KubeConfig()
  kubeConfig.loadFromDefault()
  logState.__agentsGrpcLogHelper = new Log(kubeConfig)
  return logState.__agentsGrpcLogHelper
}
  ...
  server.addService(serviceDefinition as grpc.ServiceDefinition, {
    ...
    StreamAgentRunStatus: async (call: ServerWritableStream<StatusStreamRequest, unknown>) => {
      ...
      watchHandle = startResourceWatch({
        ...
        logPrefix: '[agents][agentctl][watch]',
        onEvent: (event) => {
          if (ended) return
