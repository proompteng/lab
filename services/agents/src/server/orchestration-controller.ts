...
import { resolveOrchestrationControllerConfig } from './controller-runtime-config'
import { resolveRepositoryFromParameters } from './audit-logging'
import { isRuntimeTestEnv } from './leader-election-config'
...
const globalState = globalThis as typeof globalThis & {
  __agentsOrchestrationControllerState?: ControllerHealthState
}

const controllerState = (() => {
  if (globalState.__agentsOrchestrationControllerState) return globalState.__agentsOrchestrationControllerState
  const initial = { started: false, crdCheckState: null, namespaces: null }
  globalState.__agentsOrchestrationControllerState = initial
  return initial
})()
...
const shouldStartWithFeatureFlag = async () => {
  ...
  return resolveBooleanFeatureToggle({
    ...
    keyEnvVar: 'AGENTS_ORCHESTRATION_CONTROLLER_ENABLED_FLAG_KEY',
    fallbackEnvVar: 'AGENTS_ORCHESTRATION_CONTROLLER_ENABLED',
...
const checkCrds = async (
  ...
  if (!state.ok) {
    if (missing.length > 0) {
      console.error('[agents] missing required Orchestration CRDs:', missing.join(', '))
    }
    if (forbidden.length > 0) {
      console.error(`[agents] insufficient RBAC to list Orchestration CRDs: ${forbidden.join(', ')}`)
    }
  }
  return state
}
...
const enqueueNamespaceTask = (namespace: string, task: () => Promise<void>) => {
  ...
  const next = current
    ...
    .then(task)
    .catch((error) => {
      console.warn('[agents] orchestration controller task failed', error)
    })
...
const emitRunEvent = async (
  ...
  const event = {
    ...
    source: {
      component: 'agents-orchestration-controller',
    },
...
const reconcileAll = async (kube: ReturnType<typeof createKubernetesClient>, namespaces: string[]) => {
  ...
  try {
    ...
    console.warn('[agents] orchestration controller failed', error)
  } finally {
    reconciling = false
  }
}
...
const startNamespaceWatches = (
  ...
  handles.push(
    startResourceWatch({
      ...
      onError: (error) => console.warn('[agents] orchestration run watch failed', error),
    }),
  )
  handles.push(
    startResourceWatch({
      ...
      onError: (error) => console.warn('[agents] tool run watch failed', error),
    }),
  )
  handles.push(
    startResourceWatch({
      ...
      onError: (error) => console.warn('[agents] agent run watch failed', error),
    }),
  )
  handles.push(
    startResourceWatch({
      ...
      onError: (error) => console.warn('[agents] signal delivery watch failed', error),
    }),
  )
  handles.push(
    startResourceWatch({
      ...
      namespace,
      onEvent: handlePolicyOrSignal,
      onError: (error) => console.warn('[agents] approval policy watch failed', error),
    }),
  )
  handles.push(
    startResourceWatch({
      ...
      onError: (error) => console.warn('[agents] orchestration watch failed', error),
    }),
  )
  handles.push(
    startResourceWatch({
