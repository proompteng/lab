...
import { makeApiClientWithTransport, shouldUseBunKubernetesTransport } from './kubernetes-bun-transport'
...
const globalState = globalThis as typeof globalThis & {
  __agentsNativeKubeClients?: KubeClients
}
...
const BUILTIN_RESOURCE_TARGETS: Record<string, BuiltinResourceTarget> = {
  ...
  pvc: { apiVersion: 'v1', kind: 'PersistentVolumeClaim', plural: 'persistentvolumeclaims', namespaceScoped: true },
  pvcs: { apiVersion: 'v1', kind: 'PersistentVolumeClaim', plural: 'persistentvolumeclaims', namespaceScoped: true },
...
const getKubeClients = (): KubeClients => {
  if (globalState.__agentsNativeKubeClients) return globalState.__agentsNativeKubeClients

  const kubeConfig = new KubeConfig()
  kubeConfig.loadFromDefault()
  const useBunTransport = shouldUseBunKubernetesTransport()

  globalState.__agentsNativeKubeClients = {
    kubeConfig,
  ...
  return globalState.__agentsNativeKubeClients
}
