...
describe('orchestration controller', () => {
  beforeEach(() => {
    ...
    delete process.env.AGENTS_ORCHESTRATION_CONTROLLER_ENABLED
    delete process.env.AGENTS_ORCHESTRATION_CONTROLLER_ENABLED_FLAG_KEY
  })
  ...
  it('resolves startup gate from feature flags with env fallback default', async () => {
    ...
    try {
      ...
      process.env.AGENTS_ORCHESTRATION_CONTROLLER_ENABLED = 'false'
      ...
      expect(resolveBooleanFeatureToggleMock).toHaveBeenCalledWith({
        key: 'agents.orchestration_controller.enabled',
