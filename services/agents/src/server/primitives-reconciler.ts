...
const DEFAULT_PRIMITIVES_RECONCILER_ENABLED_FLAG_KEY = 'agents.primitives_reconciler.enabled'

export type PrimitivesReconcilerHealthState = {
  enabled: boolean
  started: boolean
  namespaces: string[]
}

type PrimitivesReconcilerState = {
  started: boolean
  namespaces: string[] | null
}

const globalState = globalThis as typeof globalThis & {
  __agentsPrimitivesReconcilerState?: PrimitivesReconcilerState
}

const reconcilerState = (() => {
  if (globalState.__agentsPrimitivesReconcilerState) return globalState.__agentsPrimitivesReconcilerState
  const initial = { started: false, namespaces: null }
  globalState.__agentsPrimitivesReconcilerState = initial
  return initial
})()

let started = reconcilerState.started
let starting = false
...
const shouldStart = async () => {
  ...
  return resolveBooleanFeatureToggle({
    ...
    keyEnvVar: 'AGENTS_PRIMITIVES_RECONCILER_FLAG_KEY',
    fallbackEnvVar: 'AGENTS_PRIMITIVES_RECONCILER',
...
const enqueueNamespaceTask = (namespace: string, task: () => Promise<void>) => {
  ...
  const next = current
    ...
    .then(task)
    .catch((error) => {
      console.warn('[agents] primitives reconciler task failed', error)
    })
...
const reconcileAgentRunItem = async (
  ...
  try {
    ...
    console.warn('[agents] failed to reconcile agent run', error)
  }
}
...
const reconcileOrchestrationRunItem = async (
  ...
  try {
    ...
    console.warn('[agents] failed to reconcile orchestration run', error)
  }
}
...
const reconcileMemories = async (
  ...
  for (const item of items) {
    try {
      ...
      console.warn('[agents] failed to reconcile memory', error)
    }
  }
}
...
const reconcileOnce = async (
  ...
  try {
    ...
    console.warn('[agents] primitives reconciler failed', error)
  } finally {
    reconciling = false
  }
}
...
export const startPrimitivesReconciler = () => {
  ...
  void (async () => {
    ...
    storeRef = store
    started = true
    reconcilerState.started = true
    reconcilerState.namespaces = namespaces
    console.info('[agents] primitives reconciler namespace scope:', JSON.stringify(namespaces))
    ...
    for (const namespace of namespaces) {
      watchHandles.push(
        startResourceWatch({
          ...
          onError: (error) => console.warn('[agents] primitives agent run watch failed', error),
        }),
      )
      watchHandles.push(
        startResourceWatch({
          ...
          onError: (error) => console.warn('[agents] primitives orchestration run watch failed', error),
        }),
      )
      watchHandles.push(
        startResourceWatch({
          ...
          onError: (error) => console.warn('[agents] primitives memory watch failed', error),
        }),
      )
    }
  })()
    .catch((error) => {
      console.error('[agents] primitives reconciler failed to start', error)
...
export const stopPrimitivesReconciler = () => {
  ...
  started = false
  reconcilerState.started = false
  reconcilerState.namespaces = null
}

export const getPrimitivesReconcilerHealth = (): PrimitivesReconcilerHealthState => {
  const config = resolvePrimitivesReconcilerConfig()
  return {
