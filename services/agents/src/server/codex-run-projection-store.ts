...
import {
  selectCodexRunProjectionById,
  selectCodexRunProjectionByAgentRun,
  selectCodexRunProjectionByAgentRunUid,
} from './codex-run-projection-rows'
...
export type CodexRunProjectionStore = {
  ready: Promise<void>
...
export const createCodexRunProjectionStore = (
  ...
  const getRunHistory = async (input: GetRunHistoryInput): Promise<CodexRunHistory> => {
    ...
    if (runIds.length > 0) {
      const artifactRows = await db
        .selectFrom('agents_codex.artifacts')
        .selectAll()
      ...
      const evaluationRows = await db
        .selectFrom('agents_codex.evaluations')
        .selectAll()
        .where('run_id', 'in', runIds)
        .orderBy('run_id')
        .orderBy('created_at desc')
  ...
  const listRecentRuns = async (input: ListRecentRunsInput): Promise<CodexRunSummaryRecord[]> => {
    ...
    let query = db
      .selectFrom('agents_codex.runs')
      .select([
        'agents_codex.runs.id as id',
        'agents_codex.runs.repository as repository',
        'agents_codex.runs.issue_number as issue_number',
        'agents_codex.runs.branch as branch',
        'agents_codex.runs.attempt as attempt',
        'agents_codex.runs.agent_run_name as agent_run_name',
        'agents_codex.runs.agent_run_namespace as agent_run_namespace',
        'agents_codex.runs.stage as stage',
        'agents_codex.runs.status as status',
        'agents_codex.runs.phase as phase',
        'agents_codex.runs.iteration as iteration',
        'agents_codex.runs.iteration_cycle as iteration_cycle',
        'agents_codex.runs.commit_sha as commit_sha',
        'agents_codex.runs.pr_number as pr_number',
        'agents_codex.runs.pr_url as pr_url',
        'agents_codex.runs.pr_state as pr_state',
        'agents_codex.runs.pr_merged as pr_merged',
        'agents_codex.runs.ci_status as ci_status',
        'agents_codex.runs.review_status as review_status',
        'agents_codex.runs.created_at as created_at',
        'agents_codex.runs.updated_at as updated_at',
        'agents_codex.runs.started_at as started_at',
        'agents_codex.runs.finished_at as finished_at',
        sql<string>`(select decision from agents_codex.evaluations e where e.run_id = agents_codex.runs.id order by e.created_at desc limit 1)`.as(
          'decision',
        ),
      ])
      .orderBy('agents_codex.runs.created_at desc')

    if (repository) {
      query = query.where('agents_codex.runs.repository', '=', repository)
    }
  ...
  const listRunsPage = async (input: ListRunsPageInput): Promise<ListRunsPageResult> => {
    ...
    let baseQuery = db.selectFrom('agents_codex.runs')
    if (repository) {
  ...
  const upsertRunComplete = async (input: UpsertRunCompleteInput) => {
    const existingById = await selectCodexRunProjectionById(db, input.runId)
    const existingByUid = existingById ? null : await selectCodexRunProjectionByAgentRunUid(db, input.agentRunUid)
    const existingByName =
      existingById || existingByUid
        ? null
        : await selectCodexRunProjectionByAgentRun(db, input.agentRunName, input.agentRunNamespace)
  ...
  const attachNotify = async (input: AttachNotifyInput) => {
    ...
    const rowById = await selectCodexRunProjectionById(db, input.runId)
    const row = rowById
      ? rowById
      : normalizedAgentRunName
        ? await selectCodexRunProjectionByAgentRun(db, normalizedAgentRunName, input.agentRunNamespace)
        : null
    if (!row) {
  ...
  const updateDecision = async (input: UpdateDecisionInput) => {
    const inserted = await db
      .insertInto('agents_codex.evaluations')
      .values({
        run_id: input.runId,
  ...
  const upsertArtifacts = async (input: UpsertArtifactsInput) => {
    ...
    for (const artifact of input.artifacts) {
      ...
      if (existing) {
        const updated = await db
          .updateTable('agents_codex.artifacts')
          .set({
            key: payload.key,
        ...
        const inserted = await db
          .insertInto('agents_codex.artifacts')
          .values(payload)
          .returningAll()
          .executeTakeFirstOrThrow()
