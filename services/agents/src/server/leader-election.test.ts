...
vi.mock('./leader-election-config', () => ({
...
const stopLeaderElectionRuntime = () => {
  const state = globalThis as typeof globalThis & {
    __agentsLeaderElection?: { stop?: () => void }
  }
  state.__agentsLeaderElection?.stop?.()
  Reflect.deleteProperty(state, '__agentsLeaderElection')
}
...
const mockInitialLeaseAcquire = () => {
  ...
  mocks.createLease.mockResolvedValueOnce({
    ...
    kind: 'Lease',
    metadata: {
      name: 'agents-controller-leader',
      namespace: 'agents',
      resourceVersion: '1',
    },
    spec: {
      holderIdentity: 'agents-0_pod-uid',
      leaseDurationSeconds: 30,
      leaseTransitions: 0,
...
describe('Agents leader election', () => {
  beforeEach(() => {
    vi.resetModules()
    vi.clearAllMocks()
    vi.useFakeTimers()
  ...
  it('labels transition logs as the Agents runtime', async () => {
    ...
    await vi.waitFor(() => {
      expect(infoSpy).toHaveBeenCalledWith(expect.stringContaining('[agents] leader election transition: leader'))
    })
  })
})
