...
func main() {
	...
	if err := run(logger); err != nil {
		logger.Error("microVM agent stopped", "error", err)
		os.Exit(1)
	}
}
...
func run(logger *slog.Logger) error {
	...
	logger.Info("microVM agent ready", "evidence", json.RawMessage(encoded))
	...
	go func() {
		logger.Info("microVM agent listening", "address", listenAddress)
