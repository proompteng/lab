...
Scope:
- Prevent pseudo-promotion artifacts from being produced when rollback readiness fails.
- Preserve deterministic hard-gating by tying patch generation and persistence promotion flags to `actuation_allowed`.
...
1. `services/torghut/app/trading/autonomy/lane.py`
   - Added explicit `actuation_allowed` computation before paper patch resolution and actuation intent build.
   - Updated paper patch generation guard to require `actuation_allowed`.
...
2. `services/torghut/tests/test_autonomous_lane.py`
   - Extended rollback-fail coverage to assert no paper patch is produced when rollback readiness is blocked.
...
- Fail path: mocked rollback readiness failure now blocks paper patch generation and prevents persisted promotion status.
