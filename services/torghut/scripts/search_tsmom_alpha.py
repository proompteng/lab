...
def _load_price_matrix(
    symbols: Iterable[str], *, start: Optional[date], end: Optional[date]
) -> pd.DataFrame:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Search TSMOM parameters and validate out-of-sample profitability."
    )
    parser.add_argument(
        "--symbols",
        required=True,
        help="Comma-separated Stooq symbols (example: spy.us,qqq.us)",
    )
    parser.add_argument(
        "--start", required=True, help="Dataset start date (YYYY-MM-DD)"
    )
    parser.add_argument(
        "--train-end",
        required=True,
        help="Train split end date (YYYY-MM-DD, inclusive)",
    )
    parser.add_argument("--end", required=True, help="Dataset end date (YYYY-MM-DD)")
    parser.add_argument(
        "--lookbacks",
        default="20,40,60,120",
        help="Comma-separated lookback day values",
    )
    parser.add_argument(
        "--vol-lookbacks",
        default="10,20,40",
        help="Comma-separated vol lookback day values",
    )
    parser.add_argument(
        "--target-vols",
        default="0.0075,0.01,0.0125",
        help="Comma-separated target daily vols",
    )
    parser.add_argument(
        "--max-grosses",
        default="0.75,1.0",
    ...
    parser.add_argument(
        "--cost-bps", type=float, default=5.0, help="Cost bps per unit turnover"
    )
    parser.add_argument(
        "--allow-shorts", action="store_true", help="Allow short exposures"
    )
    parser.add_argument(
        "--top-n", type=int, default=10, help="How many top candidates to output"
    )
