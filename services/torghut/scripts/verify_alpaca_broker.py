...
def main() -> None:
    ...
    price_lookup = (
        PriceLookup(reference_price=args.limit_price, source="explicit_limit_price")
        if args.limit_price is not None
        else _load_reference_price(client, args.symbol)
    )
    lifecycle.append(
        {
            "phase": "price_lookup",
            "status": "ok" if price_lookup.error is None else "fallback",
            "at": _utc_now(),
            "source": price_lookup.source,
            "reference_price": price_lookup.reference_price,
            "error": price_lookup.error,
        }
    )
    limit_price = round(
        max(1.0, price_lookup.reference_price * max(0.05, args.limit_discount)), 2
    )
