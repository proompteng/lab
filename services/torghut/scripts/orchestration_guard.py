...
def _validate_stage_controls(
    *,
    ...
    if stage_policy.require_previous_artifact and (
        previous_artifact is None or not previous_artifact.exists()
    ):
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Torghut v3 orchestration transition guard and retry evaluator."
    )
    parser.add_argument(
        "--policy",
        type=Path,
        default=DEFAULT_POLICY_PATH,
        help="Path to orchestration policy YAML.",
    )

    subparsers = parser.add_subparsers(dest="command", required=True)

    transition = subparsers.add_parser(
        "check-transition", help="Validate a stage transition request."
    )
    transition.add_argument(
        "--state", type=Path, required=True, help="Path to candidate state JSON."
    )
    ...
    transition.add_argument(
        "--previous-gate-passed", action="store_true", default=False
    )
    transition.add_argument(
        "--risk-controls-passed", action="store_true", default=False
    )
    transition.add_argument(
        "--execution-controls-passed", action="store_true", default=False
    )
    ...
    failure = subparsers.add_parser(
        "evaluate-failure", help="Evaluate failure class retry/pause behavior."
    )
    ...
    failure.add_argument(
        "--failure-class",
        choices=("transient", "deterministic", "spec", "policy"),
        required=True,
    )
