...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Validate Torghut v3 promotion and rollback policy checks."
    )
    parser.add_argument(
        "--policy", type=Path, required=True, help="Gate and operational policy JSON."
    )
    parser.add_argument(
        "--state", type=Path, required=True, help="Candidate state JSON."
    )
    parser.add_argument(
        "--gate-report", type=Path, required=True, help="Gate evaluation report JSON."
    )
    parser.add_argument(
        "--artifact-root",
        type=Path,
        required=True,
        help="Artifact root directory for prerequisite files.",
    )
    parser.add_argument(
