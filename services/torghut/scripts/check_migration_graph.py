...
def main() -> int:
    ...
    parent_forks = {
        ...
        for parent, children in _as_dict(
            summary.get("expected_migration_parent_forks")
        ).items()
    }
    duplicate_revisions = {
        str(revision): [str(path) for path in _as_list(paths)]
        for revision, paths in _as_dict(
            summary.get("expected_migration_duplicate_revisions")
        ).items()
    }
    orphan_parents = [
        str(parent)
    ...
    if branch_count > args.max_branch_count:
        branch_message = f"migration branch count {branch_count} exceeds allowed {args.max_branch_count}"
        if args.allow_branch_divergence:
            warnings.append(
                branch_message + "; TRADING_DB_SCHEMA_GRAPH_ALLOW_DIVERGENCE_ROOTS=true"
            )
        elif graph_signature in allowed_signatures:
            warnings.append(
                branch_message + f"; allowlisted signature {graph_signature}"
            )
        else:
            errors.append(
                branch_message
