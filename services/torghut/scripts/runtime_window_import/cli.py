"""CLI argument parsing and session setup for runtime window imports."""
...
def target_persistence_dsn(args: argparse.Namespace) -> str:
    """Get the DSN for persistence from args or environment."""
    ...
    if not target_dsn_env:
        return ""
    dsn = str(__import__("os").getenv(target_dsn_env, "")).strip()
    if not dsn:
...
@contextmanager
def persistence_session(args: argparse.Namespace) -> Iterator[sessionmaker]:
    """Create a persistence session based on args."""
    target_dsn = target_persistence_dsn(args)
    if not target_dsn:
        with SessionLocal() as session:
            yield session
        return
    engine = create_engine(
        sqlalchemy_dsn(target_dsn),
        pool_pre_ping=True,
        future=True,
    )
