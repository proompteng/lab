...
def evaluate_intraday_tsmom_signal(
    *,
    ...
    spread_ok = (
        max_spread_bps is None or spread_bps is None or spread_bps <= max_spread_bps
    )
    ...
    if isolated_strength_confirmed:
        ...
        if effective_bullish_hist_cap is not None:
            ...
            if isolated_hist_cap is not None:
                effective_bullish_hist_cap = max(
                    effective_bullish_hist_cap, isolated_hist_cap
                )
    ...
    if (
        ...
        and _optional_min_threshold(
            effective_price_drive_bps, effective_min_session_open_drive_bps
        )
        and _optional_min_threshold(
            effective_opening_window_return_bps,
        ...
        if (
            effective_bullish_hist_min is not None
            and macd_hist >= effective_bullish_hist_min * Decimal("2")
        ):
    ...
    if (
        ...
        if (
            thresholds.bearish_hist_cap is not None
            and bearish_hist > thresholds.bearish_hist_cap
        ):
            return None
        ...
        if thresholds.max_bear_rsi is not None and rsi14 <= thresholds.max_bear_rsi:
            ...
            thresholds.min_bear_rsi is not None
            and rsi14 >= thresholds.min_bear_rsi + Decimal("6")
        ):
            confidence += Decimal("0.03")
        return IntradayTsmomEvaluation(
            direction="short",
...
def _price_within_entry_band(
    *,
    ...
    if (
        min_price_below_ema12_bps is not None
        and price_gap_bps < min_price_below_ema12_bps
    ):
        return False
    if (
        max_price_below_ema12_bps is not None
        and price_gap_bps > max_price_below_ema12_bps
    ):
        return False
    return True
