    ...
    if not traces:
        return {
            fallback_family_id: {
                ...
                "filled_share": "1"
                if int(payload.get("filled_count", 0) or 0) > 0
                else "0",
            }
        }
    by_family: dict[str, dict[str, Any]] = defaultdict(
        lambda: {"evaluations": 0, "fills": 0}
    )
    total_fills = 0
    for item in traces:
...
def _decompose_entry_motifs(payload: Mapping[str, Any]) -> dict[str, dict[str, Any]]:
    traces = _trace_iter(payload)
    by_motif: dict[str, dict[str, Any]] = defaultdict(
        lambda: {"evaluations": 0, "fills": 0}
    )
    for item in traces:
        strategy_trace = cast(Mapping[str, Any], item.get("strategy_trace") or {})
        ...
        motif = str(
            context.get("entry_motif")
            or context.get("motif")
            or strategy_trace.get("action")
            or "unspecified"
        ).strip()
...
def _decompose_regime_slices(payload: Mapping[str, Any]) -> dict[str, dict[str, Any]]:
    ...
    by_regime: dict[str, dict[str, Any]] = defaultdict(
        lambda: {"evaluations": 0, "passes": 0, "fills": 0}
    )
    for item in traces:
        strategy_trace = cast(Mapping[str, Any], item.get("strategy_trace") or {})
        context = cast(Mapping[str, Any], strategy_trace.get("context") or {})
    ...
    *,
    ...
    normalized_days = {
        str(day): {
            ...
            "filled_notional": str(
                _decimal(cast(Mapping[str, Any], value).get("filled_notional"))
            ),
            "filled_count": int(
                cast(Mapping[str, Any], value).get("filled_count") or 0
            ),
        }
    ...
    family_payload = _decompose_families(
        payload=replay_payload, fallback_family_id=family_id
    )
