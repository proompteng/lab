...
def runtime_harness_payload(runtime_harness: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "disable_other_strategies": bool(
            runtime_harness.get("disable_other_strategies")
        ),
    }
...
def blocked_research_candidate_promotion_readiness(
    *,
    ...
    if (
        not resolved_runtime_harness["family"]
        or not resolved_runtime_harness["strategy_name"]
    ):
...
def summary_promotion_readiness(
