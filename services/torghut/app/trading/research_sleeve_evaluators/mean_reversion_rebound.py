...
def evaluate_mean_reversion_rebound_long(
    *,
    ...
    max_session_open_drive_bps = optional_decimal_param(
        params, "max_price_vs_session_open_bps", Decimal("-20")
    )
    max_session_range_position = optional_decimal_param(
    ...
    min_price_vs_opening_range_low_bps = optional_decimal_param(
        params, "min_price_vs_opening_range_low_bps", Decimal("-18")
    )
    max_price_vs_opening_range_low_bps = optional_decimal_param(
        params, "max_price_vs_opening_range_low_bps", Decimal("8")
    )
    ...
    min_recent_microprice_bias_bps = optional_decimal_param(
        params, "min_recent_microprice_bias_bps", Decimal("0.10")
    )
        ...
        build_gate(
            ...
            thresholds=(
                ...
                *threshold_range(
                    metric="rsi14",
                    value=rsi14,
                    floor=decimal_param(params, "min_bull_rsi", Decimal("40")),
                    ceil=decimal_param(params, "max_bull_rsi", Decimal("52")),
                    required=True,
                ),
                *threshold_range(
                    metric="macd_hist",
                    value=macd_hist,
                    floor=decimal_param(params, "bullish_hist_min", Decimal("0.002")),
    ...
    exit_reasons: dict[str, bool] = {
        "vwap_recovery": vwap_session is not None
        and price >= vwap_session
        or (vwap_session is None and price >= ema12),
        "rsi_recovered": rsi14 >= decimal_param(params, "exit_rsi_min", Decimal("61")),
        "macd_rollover": macd_hist
        <= decimal_param(params, "exit_macd_hist_max", Decimal("-0.002")),
    ...
    if exit_gate.passed:
        return build_sleeve_result(
            ...
            signal=SleeveSignalEvaluation(
                ...
                rationale=("mean_reversion_rebound_exit", "rebound_complete"),
            ),
            gates=(exit_gate,),
            trace_enabled=trace_enabled,
            context=trace_context,
