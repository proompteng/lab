...
def _resolve_tca_readiness_inputs(
    ...
    *,
    ...
    aggregate_avg_abs_slippage = _coerce_decimal(tca_summary.get('avg_abs_slippage_bps'))
    ...
    for row in rows:
        ...
        if avg_abs_slippage is not None and avg_abs_slippage <= max_allowed_slippage_bps:
    ...
    return _TcaReadinessInputs(
        ...
        last_computed_at=_latest_tca_timestamp(routeable_rows) or aggregate_last_computed_at,
        route_filter_applied=True,
    ...
    for manifest in registry.items:
        ...
        if not readiness_blockers:
            if tca_inputs.route_filter_applied and not tca_inputs.routeable_symbols:
                ...
                rollback_required = True
            elif tca_inputs.post_cost_expectancy_bps_proxy < manifest.expected_gross_edge_bps:
                ...
                if tca_inputs.order_count >= manifest.min_sample_count_for_scale_up:
                    if tca_inputs.avg_abs_slippage_bps <= manifest.max_allowed_slippage_bps * Decimal('0.60'):
                    ...
                    if tca_inputs.avg_abs_slippage_bps <= manifest.max_allowed_slippage_bps * Decimal('0.75'):
...
def summarize_hypothesis_runtime_statuses(
    ...
    *,
    ...
    return {
        ...
        'informational_reason_totals': dict(sorted(informational_reason_totals.items())),
