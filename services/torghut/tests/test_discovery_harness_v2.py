...
class TestDiscoveryHarnessV2(TestCase):
    def test_lazy_package_exports_and_attribute_errors(self) -> None:
        ...
        self.assertEqual(
            reloaded_alpha.PerformanceSummary.__name__, "PerformanceSummary"
        )
    ...
    def test_dataset_snapshot_helpers_and_transport_branches(self) -> None:
        ...
        with patch(
            ...
            with self.assertRaisesRegex(
                RuntimeError, "clickhouse_http_error: 403: bad auth"
            ):
                _http_query(
                    url="http://example.invalid",
                    username="torghut",
        ...
        completed = type(
            "Completed",
            (),
            {"returncode": 0, "stdout": "2026-05-18\n", "stderr": ""},
        )()
        with patch(
            "app.trading.discovery.dataset_snapshot.subprocess.run",
        ...
        self.assertEqual(cmd[-2:], ["--query", "SELECT 1"])
    ...
    def test_dataset_snapshot_receipt_flags_stale_and_missing_days(self) -> None:
        ...
        def fake_http_query(
            *, url: str, username: str | None, password: str | None, query: str
        ) -> str:
            del url, username, password
            return responses[query]

        with patch(
            "app.trading.discovery.dataset_snapshot._http_query",
    ...
    def test_dataset_snapshot_receipt_handles_empty_latest_days_and_override(
        ...
        self.assertEqual(
            receipt.to_payload()["witnesses"][0]["payload"]["max_event_ts"], ""
        )
    ...
    def test_family_template_loader_reads_checked_in_template(self) -> None:
        ...
        self.assertEqual(
            template.default_hard_vetoes["required_min_daily_notional"], "300000"
        )
        self.assertEqual(
            template.runtime_harness["strategy_name"],
            "breakout-continuation-long-v1",
        )
        short_template = load_family_template(
            "mean_reversion_exhaustion_short_v1", directory=root
        )
        ...
        portfolio_template = load_family_template(
            "microbar_cross_sectional_pairs_v1", directory=root
        )
        self.assertEqual(
            portfolio_template.family_id, "microbar_cross_sectional_pairs_v1"
        )
        self.assertEqual(
        ...
        late_day_template = load_family_template(
            "late_day_continuation_v1", directory=root
        )
    ...
    def test_strategy_configmap_contains_family_runtime_harness_strategies(
        self,
    ) -> None:
    ...
    def test_family_template_loader_validation_branches(self) -> None:
        with TemporaryDirectory() as tmpdir:
            ...
            with self.assertRaisesRegex(
                ValueError, "family_template_schema_invalid:wrong-schema:bad"
            ):
            ...
            with self.assertRaisesRegex(
                ValueError, "family_template_id_mismatch:mismatch"
            ):
    ...
    def test_decomposition_reports_family_symbol_and_regime_concentration(self) -> None:
        ...
        self.assertEqual(
            decomposition.entry_motifs["breakout_reclaim"]["evaluations"], 2
        )
        self.assertEqual(
            decomposition.normalization_regimes["trading_value_scaled"]["filled_count"],
            3,
        )
    ...
    def test_rank_scorecards_prefers_non_vetoed_pareto_frontier_candidates(
        self,
    ) -> None:
    ...
    def test_objective_vetoes_and_domination_cover_stale_and_dominated_branches(
        self,
    ) -> None:
