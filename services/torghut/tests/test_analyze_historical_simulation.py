...
class TestAnalyzeHistoricalSimulation(TestCase):
    ...
    def test_build_last_price_map_uses_lane_specific_price_table(self) -> None:
        ...
        with patch(
            "scripts.historical_simulation_analysis.report_helpers._http_clickhouse_query",
    ...
    def test_build_report_exports_aggregated_artifacts(self) -> None:
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            with (
                patch(
                    "scripts.historical_simulation_analysis.report_builder._load_manifest",
                    return_value={
                        "reporting": {
                            "commission_bps": "10",
                            "per_trade_fee": "0.25",
                ...
                patch(
                    "scripts.historical_simulation_analysis.report_builder._build_resources",
                    return_value=resources,
                ),
                patch(
                    "scripts.historical_simulation_analysis.report_builder._default_simulation_postgres_db",
                    return_value="simulation_db",
                ),
                patch(
                    "scripts.historical_simulation_analysis.report_builder._build_postgres_runtime_config",
                    return_value=postgres_config,
                ),
                patch(
                    "scripts.historical_simulation_analysis.report_builder._build_clickhouse_runtime_config",
                ...
                patch(
                    "scripts.historical_simulation_analysis.report_builder._resolve_window_bounds",
                    return_value=(
                ...
                patch(
                    "scripts.historical_simulation_analysis.report_builder._validate_window_policy",
                    return_value={"strict_coverage_ratio": 0.4},
                ),
                patch(
                    "scripts.historical_simulation_analysis.report_builder.psycopg.connect",
                    return_value=FakeConnection(),
                ),
                patch(
                    "scripts.historical_simulation_analysis.report_builder._query_rows",
                    side_effect=_fake_query_rows,
                ),
                patch(
                    "scripts.historical_simulation_analysis.report_builder._collect_clickhouse_stats",
