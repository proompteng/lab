...
class TestGovernancePolicyDryRun(TestCase):
    def test_dry_run_blocks_progression_when_artifact_missing(self) -> None:
        ...
        self.assertIn('paper-candidate/strategy-configmap-patch.yaml', output['promotion_prerequisites']['missing_artifacts'])
    ...
    def test_dry_run_blocks_progression_when_benchmark_parity_artifact_missing(self) -> None:
        output = self._run_harness('--simulate-missing-benchmark-parity-artifact')
        self.assertFalse(output['promotion_progression_allowed'])
        reasons = output['promotion_prerequisites']['reasons']
        self.assertIn('required_artifacts_missing', reasons)
        self.assertIn('benchmark_parity_report_missing', reasons)

    def test_dry_run_blocks_progression_when_benchmark_parity_artifact_invalid(self) -> None:
        output = self._run_harness('--simulate-invalid-benchmark-parity-artifact')
        self.assertFalse(output['promotion_progression_allowed'])
        reasons = output['promotion_prerequisites']['reasons']
        self.assertIn('benchmark_parity_schema_invalid', reasons)

    def test_dry_run_blocks_progression_when_benchmark_parity_threshold_breached(self) -> None:
        output = self._run_harness('--simulate-benchmark-parity-threshold-breach')
        self.assertFalse(output['promotion_progression_allowed'])
        reasons = output['promotion_prerequisites']['reasons']
        self.assertIn('benchmark_parity_advisory_output_rate_below_minimum', reasons)
