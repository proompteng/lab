...
class TestForecastRouterV5(TestCase):
    def test_router_emits_forecast_contract_v1_with_uncertainty_and_interval(
        self,
    ) -> None:
        router = build_default_forecast_router(
            policy_path=None, refinement_enabled=True
        )
        signal = _signal()
    ...
    def test_router_ignores_invalid_regime_id_and_falls_back_to_explicit_label(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            policy_path = Path(tmpdir) / "router-policy.json"
    ...
    def test_router_uses_legacy_regime_block_for_fallback_when_hmm_regime_is_invalid(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            policy_path = Path(tmpdir) / "router-policy.json"
    ...
    def test_router_ignores_transition_shock_hmm_regime_for_route_matching(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            policy_path = Path(tmpdir) / "router-policy.json"
    ...
    def test_router_prefers_explicit_route_regime_label_hint_when_hmm_invalid(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            policy_path = Path(tmpdir) / "router-policy.json"
