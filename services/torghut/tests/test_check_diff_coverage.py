...
from scripts.check_diff_coverage import (
    FileDiffCoverage,
    drop_missing_non_executable_files as _drop_missing_non_executable_files,
    executable_source_lines as _executable_source_lines,
    format_summary as _format_summary,
    git as _git,
    git_optional as _git_optional,
    include_untracked_python_files as _include_untracked_python_files,
    load_coverage_index as _load_coverage_index,
    list_untracked_python_files as _list_untracked_python_files,
    parse_args as _parse_args,
    parse_changed_python_lines as _parse_changed_python_lines,
    repo_root as _repo_root,
    resolve_base_spec as _resolve_base_spec,
    resolve_diff_base as _resolve_diff_base,
    main,
...
class TestCheckDiffCoverage(TestCase):
    def test_file_diff_coverage_ratio_defaults_to_one_for_zero_executable_lines(
        self,
    ) -> None:
    ...
    def test_git_optional_returns_none_on_called_process_error(
        self, git_mock: object
    ) -> None:
...
""".strip()
    ...
    def test_resolve_diff_base_prefers_origin_main_when_no_explicit_base(
        self, git_optional: object
    ) -> None:
    ...
    @patch.dict("os.environ", {}, clear=True)
    @patch("scripts.check_diff_coverage._git_optional")
    def test_resolve_diff_base_falls_back_to_head_parent(
        self, git_optional: object
    ) -> None:
    ...
    @patch("scripts.check_diff_coverage._git_optional")
    def test_list_untracked_python_files_filters_tracked_prefixes(
        self, git_optional: object
    ) -> None:
    ...
    @patch("scripts.check_diff_coverage._git_optional")
    def test_list_untracked_python_files_returns_empty_tuple_when_git_has_no_output(
        self, git_optional: object
    ) -> None:
        git_optional.return_value = ""
    ...
    def test_include_untracked_python_files_uses_coverage_lines_when_present(
        self, list_untracked: object
    ) -> None:
    ...
    @patch("scripts.check_diff_coverage._list_untracked_python_files")
    def test_include_untracked_python_files_falls_back_to_ast_lines(
        self, list_untracked: object
    ) -> None:
    ...
    def test_drop_missing_non_executable_files_keeps_only_executable_changes(
        self,
    ) -> None:
        with TemporaryDirectory() as tmpdir:
            service_root = Path(tmpdir)
            empty_path = service_root / "app" / "__init__.py"
            empty_path.parent.mkdir(parents=True, exist_ok=True)
            empty_path.write_text("", encoding="utf-8")
            executable_path = service_root / "scripts" / "new_tool.py"
            executable_path.parent.mkdir(parents=True, exist_ok=True)
            executable_path.write_text("def main() -> int:\n    return 0\n")

            filtered = _drop_missing_non_executable_files(
                changed_lines={
                    "app/__init__.py": {1},
                    "scripts/new_tool.py": {1, 2},
                    "app/covered.py": {1},
                },
                coverage_index={"app/covered.py": {1: 1}},
                service_root=service_root,
            )
    ...
    def test_summarize_changed_coverage_skips_non_executable_changed_lines(
        self,
    ) -> None:
        summary = summarize_changed_coverage(
    ...
    def test_main_skips_when_no_base_commit(
        ...
        self.assertTrue(
            any(
                "no base commit available" in call.args[0]
    ...
    def test_main_skips_when_no_changed_python_source_lines(
        ...
        self.assertTrue(
            any(
                "no changed Torghut Python source lines" in call.args[0]
                for call in stdout_write.call_args_list
            )
        )
    ...
    def test_main_fails_for_missing_coverage_files(
        ...
        self.assertTrue(
            any(
                "missing from coverage.xml" in call.args[0]
    ...
    def test_main_fails_when_threshold_not_met(
        ...
        self.assertTrue(
            any(
                "below threshold 95.00%" in call.args[0]
                for call in stderr_write.call_args_list
            )
        )
    ...
    def test_main_succeeds_when_threshold_is_met(
        ...
        parse_args.return_value = Mock(
            coverage_xml="coverage.xml", threshold=50.0, base_ref=""
        )
        repo_root.return_value = Path("/tmp/repo")
        resolve_diff_base.return_value = "base"
        git_mock.return_value = ""
