...
def test_kafka_clickhouse_writer_replay_is_contained_and_bounded() -> None:
    ...
    assert data.get("CLICKHOUSE_WRITER_MAX_POLL_RECORDS") == "1000"
    assert data.get("CLICKHOUSE_WRITER_HIGH_THROUGHPUT_BATCH_SIZE") == "1000"
    assert data.get("CLICKHOUSE_WRITER_SPARSE_BATCH_SIZE") == "100"
    assert data.get("CLICKHOUSE_WRITER_MAX_BUFFERED_RECORDS_PER_PARTITION") == "2000"
