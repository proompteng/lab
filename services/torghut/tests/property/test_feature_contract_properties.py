...
def test_normalization_preserves_rank_and_ratio_bounds(signal) -> None:
    ...
    assert (
        Decimal("0")
        <= feature_vector.values["recent_quote_invalid_ratio"]
        <= Decimal("1")
    )
    assert (
        Decimal("0")
        <= feature_vector.values["cross_section_prev_session_close_rank"]
        <= Decimal("1")
    )
    assert (
        Decimal("0")
        <= feature_vector.values["cross_section_positive_session_open_ratio"]
        <= Decimal("1")
    )
