...
class TestPromotionTruthfulness(TestCase):
    ...
    def test_promotion_prerequisites_fail_when_simulation_calibration_is_uncalibrated(
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            artifact_root = Path(tmpdir)
            report_path = (
                artifact_root / "gates" / "simulation-calibration-report-v1.json"
            )
    ...
    def test_promotion_prerequisites_fail_when_shadow_live_deviation_exceeds_budget(
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            artifact_root = Path(tmpdir)
            report_path = (
                artifact_root / "gates" / "shadow-live-deviation-report-v1.json"
            )
            report_path.parent.mkdir(parents=True, exist_ok=True)
            report_path.write_text(
                json.dumps(
    ...
    def test_probation_evidence_collection_never_becomes_promotion_authority(
        ...
        self.assertEqual(
            readiness["final_authority_contract"]["min_closed_trades"], 300
        )
