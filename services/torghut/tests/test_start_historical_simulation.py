...
class TestStartHistoricalSimulation(TestCase):
    ...
    def test_run_full_lifecycle_runs_teardown_analysis_after_argocd_restore(self) -> None:
        ...
        with ExitStack() as stack:
            ...
            mock_session_local = stack.enter_context(patch('scripts.start_historical_simulation.SessionLocal'))
            stack.enter_context(
                patch('scripts.start_historical_simulation._read_argocd_automation_mode', return_value={'mode': 'auto'}),
            )
            stack.enter_context(
                patch(
    ...
    def test_run_full_lifecycle_fails_before_teardown_clean_when_teardown_settle_not_ready(self) -> None:
        resources = _build_resources(
    ...
    def test_wait_for_torghut_service_revision_ready_returns_unready_report_on_timeout(self) -> None:
        resources = _build_resources(
    ...
    def test_simulation_analysis_templates_do_not_pass_unsupported_forecast_service_arg(self) -> None:
        templates_dir = Path(__file__).resolve().parents[3] / 'argocd' / 'applications' / 'torghut'
        template_names = [
            'analysis-template-runtime-ready.yaml',
        ...
        for template_name in template_names:
            with self.subTest(template_name=template_name):
                template = yaml.safe_load((templates_dir / template_name).read_text(encoding='utf-8'))
                spec = template['spec']
                arg_names = [entry['name'] for entry in spec['args']]
                args_text = spec['metrics'][0]['provider']['job']['spec']['template']['spec']['containers'][0]['args'][0]
    ...
    def test_restore_argocd_after_run_restores_root_and_applicationset(self) -> None:
        ...
        applicationset_mock.assert_called_once()
        self.assertEqual(application_mock.call_args_list, [
            call(
                config=config,
                app_name='torghut',
