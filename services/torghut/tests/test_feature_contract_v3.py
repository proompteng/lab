...
class TestFeatureContractV3(TestCase):
    ...
    def test_normalization_prefers_executable_price_over_nested_vwap(self) -> None:
        ...
        self.assertEqual(
            ...
            Decimal("149.2537313432835820895522388"),
        )

    def test_normalization_prefers_top_level_imbalance_midpoint_over_vwap_keys(
        self,
    ) -> None:
        signal = SignalEnvelope(
            event_ts=datetime(2026, 1, 1, 0, 0, tzinfo=timezone.utc),
    ...
    def test_normalization_maps_session_context_and_derived_vwap_fields(self) -> None:
        ...
        fv = normalize_feature_vector_v3(signal)
        self.assertEqual(
            fv.values["imbalance_pressure"], Decimal("0.08510638297872340425531914894")
        )
        self.assertEqual(
            fv.values["vwap_w5m_vs_session_bps"],
            Decimal("19.84126984126984126984126984"),
        )
        self.assertEqual(
            fv.values["price_vs_vwap_w5m_bps"], Decimal("19.80198019801980198019801980")
        )
        ...
        self.assertEqual(
            fv.values["price_vs_opening_window_close_bps"], Decimal("29.73")
        )
        ...
        self.assertEqual(
            fv.values["clusterlob_event_cluster_stability_score"], Decimal("0.74")
        )
        self.assertEqual(
        ...
        self.assertEqual(
            fv.values["cross_section_prev_session_close_rank"], Decimal("0.91")
        )
        self.assertEqual(
            fv.values["cross_section_positive_session_open_ratio"], Decimal("0.58")
        )
        self.assertEqual(
            fv.values["cross_section_positive_prev_session_close_ratio"],
            Decimal("0.75"),
        )
        self.assertEqual(
            fv.values["cross_section_positive_opening_window_return_ratio"],
            Decimal("0.66"),
        )
        self.assertEqual(
            fv.values[
        ...
        self.assertEqual(
            fv.values["cross_section_above_vwap_w5m_ratio"], Decimal("0.50")
        )
        self.assertEqual(
            fv.values["cross_section_positive_recent_imbalance_ratio"], Decimal("0.75")
        )
        self.assertEqual(
            fv.values["cross_section_continuation_breadth"], Decimal("0.6225")
        )
        self.assertEqual(
            fv.values["cross_section_vwap_w5m_stretch_rank"], Decimal("0.14")
        )
        self.assertEqual(
            fv.values["cross_section_recent_15m_return_rank"], Decimal("0.76")
        )
        self.assertEqual(
            fv.values["cross_section_microbar_volume_rank"], Decimal("0.88")
        )
        ...
        self.assertEqual(
            fv.values["cross_section_opening_window_return_rank"], Decimal("0.83")
        )
        self.assertEqual(
    ...
    def test_normalization_persists_hmm_transition_shock_and_artifact_lineage(
        self,
    ) -> None:
        signal = SignalEnvelope(
        ...
        self.assertEqual(
            feature_vector.values["hmm_artifact_model_id"], "hmm-regime-v1.2.0"
        )
        self.assertEqual(
            feature_vector.values["hmm_artifact_feature_schema"],
            "hmm-v1-feature-schema",
        )
        self.assertEqual(
            feature_vector.values["hmm_artifact_training_run_id"], "trn_2026-02-28"
        )
