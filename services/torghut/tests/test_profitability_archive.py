...
class TestProfitabilityArchive(TestCase):
    def test_archive_historical_simulation_run_copies_source_dump_and_writes_bundle_contracts(
        self,
    ) -> None:
        with TemporaryDirectory() as tmpdir:
            ...
            replay_manifest = json.loads(
                (archive_dir / "replay_day_manifest.json").read_text(encoding="utf-8")
            )
            self.assertEqual(
                replay_manifest["simulation_database"], "torghut_sim_sim-2026-03-27"
            )
            self.assertEqual(replay_manifest["archive_status"], ARCHIVE_STATUS_ARCHIVED)

            market_day_inventory = json.loads(
                (archive_dir / "market_day_inventory.json").read_text(encoding="utf-8")
            )
            self.assertTrue(market_day_inventory["replayable_market_day"])

            execution_day_inventory = json.loads(
                (archive_dir / "execution_day_inventory.json").read_text(
                    encoding="utf-8"
                )
            )
    ...
    def test_data_sufficiency_marks_short_archive_insufficient(self) -> None:
        ...
        self.assertIn(
            "replay_ready_market_days_below_research_minimum", summary["blockers"]
        )
