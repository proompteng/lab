...
class TestFeatureParity(TestCase):
    ...
    def test_parity_report_detects_drift_and_is_machine_readable(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            output_path = write_feature_parity_report(
    ...
    def test_deeplob_bdlob_report_includes_standardized_contract(self) -> None:
        ...
        self.assertEqual(
            report.get("overall_status"), "blocked_missing_empirical_authority"
        )
        self.assertEqual(
    ...
    def test_deterministic_scaffold_reports_fail_closed_without_empirical_authority(
        self,
    ) -> None:
        now = datetime(2026, 3, 3, tzinfo=timezone.utc)
        ...
        self.assertEqual(
            foundation.get("overall_status"), "blocked_missing_empirical_authority"
        )
        self.assertEqual(
        ...
        self.assertEqual(
            advisor.get("overall_status"), "blocked_missing_empirical_authority"
        )
        self.assertEqual(
