...
class TestSignalIngestPart2(_TestSignalIngestBase):
    def test_fetch_signals_between_dedupes_replay_duplicates_by_stable_identity(
        ...
        self.assertEqual(
            signals[0].event_ts, datetime(2026, 3, 6, 14, 50, tzinfo=timezone.utc)
        )
    ...
    def test_fetch_signals_between_preserves_distinct_envelopes_sharing_timestamp(
        self,
    ) -> None:
        rows = [
            {
        ...
        ingestor = DistinctEnvelopeIngestor(
            schema="envelope", table="torghut.ta_signals", url="http://example"
        )
        signals = ingestor.fetch_signals_between(
            start=datetime(2026, 3, 6, 14, 50, tzinfo=timezone.utc),
    ...
    def test_fetch_signals_between_filters_disallowed_sources(self) -> None:
        ...
        try:
            ...
            ingestor = SourceFilteringIngestor(
                schema="envelope", table="torghut.ta_signals", url="http://example"
            )
            signals = ingestor.fetch_signals_between(
                start=datetime(2026, 3, 6, 14, 50, tzinfo=timezone.utc),
    ...
    def test_latest_signal_timestamp_prefers_descending_timestamp_probe_for_event_ts(
        self,
    ) -> None:
        latest = datetime(2026, 1, 1, tzinfo=timezone.utc)
    ...
    def test_latest_signal_timestamp_uses_cached_value_on_refresh_failure(self) -> None:
        ...
        first = ingestor._latest_signal_timestamp("event_ts")
        ingestor._latest_signal_ts_checked_at = datetime.now(timezone.utc) - timedelta(
            seconds=31
        )
        second = ingestor._latest_signal_timestamp("event_ts")
    ...
    def test_simulation_fetch_query_is_cursor_ordered_limited_and_bounded(self) -> None:
        ...
        self.assertEqual(
            [(signal.symbol, signal.seq) for signal in batch.signals], [("AVGO", 22894)]
        )

    def test_simulation_fetch_query_keeps_equal_timestamp_without_tiebreakers(
        self,
    ) -> None:
        ingestor = SimulationWindowIngestor(
            schema="envelope",
    ...
    def test_cursor_is_account_scoped(self) -> None:
        ...
        with session_local() as session:
            ...
            rows = (
                session.execute(select(TradeCursor).order_by(TradeCursor.account_label))
                .scalars()
                .all()
            )
    ...
    def test_empty_fetch_advances_cursor_to_recent_window(self) -> None:
        ...
        with session_local() as session:
            session.add(
                TradeCursor(
                    source="clickhouse", cursor_at=stale_cursor, cursor_seq=None
                )
            )
            session.commit()
    ...
    def test_cursor_ahead_of_signal_stream_rewinds_and_reasons(self) -> None:
        ...
        with session_local() as session:
            session.add(
                TradeCursor(source="clickhouse", cursor_at=cursor_time, cursor_seq=None)
            )
            session.commit()
    ...
    def test_flat_cursor_overlap_dedupes_older_rows(self) -> None:
        ...
        with session_local() as session:
            cursor = TradeCursor(
                source="clickhouse", cursor_at=base_ts, cursor_seq=None
            )
            session.add(cursor)
            session.commit()
            batch = ingestor.fetch_signals(session)
