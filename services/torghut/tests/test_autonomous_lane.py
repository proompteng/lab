...
class TestAutonomousLane(TestCase):
    ...
    @staticmethod
    def _parity_artifact_policy() -> dict[str, object]:
        return {
            ...
            "gate6_require_benchmark_parity_evidence": True,
            "promotion_require_benchmark_parity_evidence": True,
            ...
            "promotion_benchmark_parity_schema_version": "benchmark-parity-report-v1",
        }

    @staticmethod
    def _write_valid_benchmark_parity_payload(
        output_path: Path,
        *,
        advisory_output_rate: float = 1.0,
        schema_version: str = "benchmark-parity-report-v1",
        overall_parity_status: str = "pass",
    ) -> None:
        payload = {
            "schema_version": schema_version,
            "candidate_id": "cand-autonomy-lane",
            "baseline_candidate_id": "baseline-legacy-macd-rsi",
            "benchmark_runs": {
                "ai_trader_like": {
                    "dataset_ref": "walkforward:autonomy",
                    "window_ref": "run:autonomy-lane",
                    "metrics": {
            ...
            "overall_parity_status": overall_parity_status,
            "degradation_summary": {
                "policy_violation_rate_delta": 0.0,
                "adverse_regime_decision_quality_delta": 0.0,
            ...
            "artifact_hash": "parity-artifact-hash",
    ...
    def test_lane_emits_gate_report_and_paper_patch(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertTrue(benchmark_parity_report_path.exists())
            ...
            self.assertIn(str(benchmark_parity_report_path), actuation_payload["artifact_refs"])
            self.assertIn("benchmark_parity", evidence)
            self.assertEqual(
    ...
    def test_lane_progression_and_iteration_notes_respect_execution_artifact_path(
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = run_autonomous_lane(
                ...
                governance_inputs={
                    "execution_context": {
                        ...
                        "base": "feature/base",
                        "head": "run/head",
                        "designDoc": "docs/design/override-repo.md",
                        "priorityId": "P-1001",
            ...
            self.assertIn("designDoc: docs/design/override-repo.md", note_text)
            self.assertFalse(
            ...
            self.assertEqual(
                phase_manifest["execution_context"]["designDoc"],
                "docs/design/override-repo.md",
            )
            actuation_payload = json.loads(
                result.actuation_intent_path.read_text(encoding="utf-8")
                if result.actuation_intent_path
            ...
            self.assertEqual(
                actuation_payload["governance"]["design_doc"],
                "docs/design/override-repo.md",
            )
    ...
    def test_lane_top_level_execution_context_args_are_honored(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = run_autonomous_lane(
                ...
                priority_id="P-2002",
                design_doc="docs/design/top-level.md",
            )
            ...
            self.assertIn("designDoc: docs/design/top-level.md", note_text)
            self.assertFalse(
                any((output_dir / "notes").glob("iteration-*.md")),
    ...
    @patch("app.trading.autonomy.lane.write_benchmark_parity_report_v1")
    def test_lane_fails_closed_when_benchmark_parity_artifact_is_missing(
        self,
        mock_write_parity: object,
    ) -> None:
        fixture_path = Path(__file__).parent / "fixtures" / "walkforward_signals.json"
        strategy_config_path = (
            Path(__file__).parent.parent / "config" / "autonomous-strategy-sample.yaml"
        ...
        mock_write_parity.side_effect = lambda *args, **kwargs: None

        with tempfile.TemporaryDirectory() as tmpdir:
            output_dir = Path(tmpdir) / "lane-benchmark-parity-missing"
            policy_path = Path(tmpdir) / "policy.json"
            policy_path.write_text(
                json.dumps(self._parity_artifact_policy()),
                encoding="utf-8",
            )
            result = run_autonomous_lane(
                signals_path=fixture_path,
        ...
        self.assertIn("required_artifacts_missing", promotion_reqs["reasons"])
        self.assertIn(
            "benchmark_parity_report_missing",
            promotion_reqs["reasons"],
        )
        self.assertIn("required_artifacts_missing", actuation_payload["gates"]["recommendation_reasons"])

    @patch("app.trading.autonomy.lane.write_benchmark_parity_report_v1")
    def test_lane_fails_closed_when_benchmark_parity_artifact_is_invalid(
        self,
        mock_write_parity: object,
    ) -> None:
        fixture_path = Path(__file__).parent / "fixtures" / "walkforward_signals.json"
        ...
        def _write_invalid_parity(*_args: Any, **_kwargs: Any) -> None:
            output_path = Path(_args[1]) if len(_args) > 1 else Path(_kwargs["output_path"])
            payload = {"schema_version": "invalid-schema-version"}
            output_path.write_text(json.dumps(payload), encoding="utf-8")

        mock_write_parity.side_effect = _write_invalid_parity

        with tempfile.TemporaryDirectory() as tmpdir:
            output_dir = Path(tmpdir) / "lane-benchmark-parity-invalid"
            policy_path = Path(tmpdir) / "policy.json"
            policy_path.write_text(
                json.dumps(self._parity_artifact_policy()),
                encoding="utf-8",
        ...
        self.assertFalse(actuation_payload["actuation_allowed"])
        self.assertIn("benchmark_parity_schema_invalid", promotion_reqs["reasons"])
        self.assertIn(
            "benchmark_parity_schema_invalid",
            actuation_payload["gates"]["recommendation_reasons"],
        )

    @patch("app.trading.autonomy.lane.write_benchmark_parity_report_v1")
    def test_lane_fails_closed_when_benchmark_parity_threshold_breached(
        self,
        mock_write_parity: object,
    ) -> None:
        fixture_path = Path(__file__).parent / "fixtures" / "walkforward_signals.json"
        ...
        def _write_low_advisory_parity(*args: Any, **_kwargs: Any) -> None:
            self._write_valid_benchmark_parity_payload(
                output_path=args[1],
                advisory_output_rate=0.75,
                overall_parity_status="pass",
            )

        mock_write_parity.side_effect = _write_low_advisory_parity

        with tempfile.TemporaryDirectory() as tmpdir:
            output_dir = Path(tmpdir) / "lane-benchmark-parity-threshold"
            policy_path = Path(tmpdir) / "policy.json"
            policy_path.write_text(
                json.dumps(self._parity_artifact_policy()),
                encoding="utf-8",
