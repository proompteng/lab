...
def upgrade() -> None:
    ...
    if "execution_correlation_id" not in execution_columns:
        op.add_column(
            "executions",
            sa.Column("execution_correlation_id", sa.String(length=64), nullable=True),
        )
    if "execution_idempotency_key" not in execution_columns:
        op.add_column(
            "executions",
            sa.Column("execution_idempotency_key", sa.String(length=96), nullable=True),
        )
    ...
    if "ix_executions_execution_correlation_id" not in execution_indexes:
        op.create_index(
            "ix_executions_execution_correlation_id",
            "executions",
    ...
    if not _table_exists(inspector, "lean_backtest_runs"):
        ...
        op.create_index(
            "ix_lean_backtest_runs_status", "lean_backtest_runs", ["status"]
        )
        op.create_index("ix_lean_backtest_runs_lane", "lean_backtest_runs", ["lane"])
        op.create_index(
            "ix_lean_backtest_runs_created_at", "lean_backtest_runs", ["created_at"]
        )
    ...
    if not _table_exists(inspector, "lean_execution_shadow_events"):
        ...
        op.create_index(
            "ix_lean_execution_shadow_events_created_at",
            "lean_execution_shadow_events",
            ["created_at"],
        )
        op.create_index(
            "ix_lean_execution_shadow_events_symbol",
            "lean_execution_shadow_events",
            ["symbol"],
        )
        op.create_index(
            "ix_lean_execution_shadow_events_status",
            "lean_execution_shadow_events",
            ["parity_status"],
        )
        op.create_index(
    ...
    if not _table_exists(inspector, "lean_canary_incidents"):
        op.create_table(
            ...
            sa.Column(
                "incident_key", sa.String(length=96), nullable=False, unique=True
            ),
            ...
            sa.Column(
                "symbols", postgresql.JSONB(astext_type=sa.Text()), nullable=True
            ),
            sa.Column(
                "evidence_json", postgresql.JSONB(astext_type=sa.Text()), nullable=True
            ),
        ...
        op.create_index(
            "ix_lean_canary_incidents_breach_type",
            "lean_canary_incidents",
            ["breach_type"],
        )
        op.create_index(
            "ix_lean_canary_incidents_created_at",
            "lean_canary_incidents",
            ["created_at"],
        )
    ...
    if not _table_exists(inspector, "lean_strategy_shadow_evaluations"):
        op.create_table(
            ...
            sa.Column("symbol", sa.String(length=32), nullable=False),
            sa.Column(
                "intent_json", postgresql.JSONB(astext_type=sa.Text()), nullable=False
            ),
            sa.Column(
                "shadow_json", postgresql.JSONB(astext_type=sa.Text()), nullable=True
            ),
            sa.Column(
                "parity_status",
                sa.String(length=32),
        ...
        op.create_index(
            "ix_lean_strategy_shadow_evaluations_strategy",
            "lean_strategy_shadow_evaluations",
            ["strategy_id"],
        )
        op.create_index(
            "ix_lean_strategy_shadow_evaluations_symbol",
            "lean_strategy_shadow_evaluations",
            ["symbol"],
        )
        op.create_index(
            "ix_lean_strategy_shadow_evaluations_status",
            "lean_strategy_shadow_evaluations",
            ["parity_status"],
        )
