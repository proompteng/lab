...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_research_runs_gate_trace", "research_runs", ["gate_report_trace_id"]
    )
    op.create_index(
        "ix_research_runs_recommendation_trace",
        "research_runs",
...
def downgrade() -> None:
    ...
    op.drop_constraint(
        "ck_executions_fallback_reason_required", "executions", type_="check"
    )
    op.alter_column(
        "executions",
        "execution_actual_adapter",
