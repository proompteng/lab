...
def upgrade() -> None:
    ...
    if "order_feed_last_event_ts" not in execution_columns:
        op.add_column(
            "executions",
            sa.Column(
                "order_feed_last_event_ts", sa.DateTime(timezone=True), nullable=True
            ),
        )
    if "order_feed_last_seq" not in execution_columns:
        op.add_column(
            "executions",
            sa.Column("order_feed_last_seq", sa.BigInteger(), nullable=True),
        )
    ...
    if "ix_executions_order_feed_last_event_ts" not in execution_indexes:
        op.create_index(
            "ix_executions_order_feed_last_event_ts",
            "executions",
            ["order_feed_last_event_ts"],
        )
    ...
    if not inspector.has_table("execution_order_events"):
        op.create_table(
            ...
            sa.Column(
                "event_fingerprint", sa.String(length=64), nullable=False, unique=True
            ),
        ...
        op.create_index(
            "ix_execution_order_events_event_ts", "execution_order_events", ["event_ts"]
        )
        op.create_index(
            "ix_execution_order_events_execution_id",
            "execution_order_events",
            ["execution_id"],
        )
        op.create_index(
        ...
        op.create_index(
            "ix_execution_order_events_client_order_id",
            "execution_order_events",
            ["client_order_id"],
        )
        op.create_index(
...
def downgrade() -> None:
    ...
    if inspector.has_table("execution_order_events"):
        op.execute(
            "REVOKE ALL PRIVILEGES ON TABLE public.execution_order_events FROM torghut_app;"
        )
        op.drop_index(
            "uq_execution_order_events_source_offset",
        ...
        op.drop_index(
            "ix_execution_order_events_order_id", table_name="execution_order_events"
        )
        op.drop_index(
            "ix_execution_order_events_trade_decision_id",
            table_name="execution_order_events",
        ...
        op.drop_index(
            "ix_execution_order_events_event_ts", table_name="execution_order_events"
        )
