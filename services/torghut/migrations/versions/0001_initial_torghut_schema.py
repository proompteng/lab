...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_trade_decisions_strategy", "trade_decisions", ["strategy_id"], unique=False
    )
    op.create_index(
        "ix_trade_decisions_status", "trade_decisions", ["status"], unique=False
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "alpaca_order_id", sa.String(length=128), nullable=False, unique=True
        ),
        ...
        sa.Column(
            "filled_qty", sa.Numeric(20, 8), nullable=False, server_default=sa.text("0")
        ),
    ...
    op.create_index(
        "ix_executions_alpaca_order_id", "executions", ["alpaca_order_id"], unique=False
    )
    op.create_index(
        "ix_executions_symbol_status", "executions", ["symbol", "status"], unique=False
    )
    ...
    op.create_index(
        "ix_position_snapshots_as_of", "position_snapshots", ["as_of"], unique=False
    )
    op.create_index(
    ...
    op.create_index(
        "ix_tool_run_logs_codex_session",
        "tool_run_logs",
        ["codex_session_id"],
        unique=False,
    )
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_position_snapshots_account_label", table_name="position_snapshots"
    )
