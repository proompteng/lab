...
def _ensure_execution_columns(inspector: sa.Inspector) -> None:
    ...
    column_specs: tuple[tuple[str, sa.Column[Any]], ...] = (
        (
            ...
            sa.Column(
                "execution_expected_adapter", sa.String(length=32), nullable=True
            ),
        ),
        (
        ...
        (
            ...
            sa.Column(
                "execution_fallback_reason", sa.String(length=128), nullable=True
            ),
        ),
        (
...
def _create_research_candidates_table() -> None:
    op.create_table(
        ...
        sa.Column(
            "decision_count", sa.BigInteger(), nullable=False, server_default="0"
        ),
    ...
    op.create_index(
        "ix_research_candidates_candidate_id", "research_candidates", ["candidate_id"]
    )
...
def _create_research_fold_metrics_table() -> None:
    ...
    op.create_index(
        "ix_research_fold_metrics_candidate", "research_fold_metrics", ["candidate_id"]
    )
    op.create_index(
        "ix_research_fold_metrics_fold_order", "research_fold_metrics", ["fold_order"]
    )
...
def _create_research_stress_metrics_table() -> None:
    ...
    op.create_index(
        "ix_research_stress_metrics_candidate",
        "research_stress_metrics",
        ["candidate_id"],
    )
    op.create_index(
        "ix_research_stress_metrics_case", "research_stress_metrics", ["stress_case"]
    )
...
def _create_research_promotions_table() -> None:
    ...
    op.create_index(
        "ix_research_promotions_candidate", "research_promotions", ["candidate_id"]
    )
    op.create_index(
    ...
    op.create_index(
        "ix_research_promotions_approved_mode", "research_promotions", ["approved_mode"]
    )
...
def _drop_table_with_indexes(
    inspector: sa.Inspector, table: str, indexes: tuple[str, ...]
) -> None:
...
def _drop_execution_indexes(inspector: sa.Inspector) -> None:
    execution_indexes = _index_names(inspector, "executions")
    for index_name in (
        "ix_executions_expected_adapter",
        "ix_executions_actual_adapter",
    ):
