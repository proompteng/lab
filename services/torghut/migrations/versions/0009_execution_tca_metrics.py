...
def upgrade() -> None:
    ...
    if not inspector.has_table("execution_tca_metrics"):
        op.create_table(
            ...
            sa.Column(
                "filled_qty", sa.Numeric(20, 8), nullable=False, server_default="0"
            ),
            sa.Column(
                "signed_qty", sa.Numeric(20, 8), nullable=False, server_default="0"
            ),
            sa.Column("slippage_bps", sa.Numeric(20, 8), nullable=True),
            sa.Column("shortfall_notional", sa.Numeric(20, 8), nullable=True),
            sa.Column(
                "churn_qty", sa.Numeric(20, 8), nullable=False, server_default="0"
            ),
        ...
        op.create_index(
            "ix_execution_tca_metrics_symbol", "execution_tca_metrics", ["symbol"]
        )
        op.create_index(
            "ix_execution_tca_metrics_computed_at",
            "execution_tca_metrics",
            ["computed_at"],
        )
    ...
    op.execute(
        "GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE public.execution_tca_metrics TO torghut_app;"
    )
    op.execute(
...
def downgrade() -> None:
    ...
    op.execute(
        "REVOKE ALL PRIVILEGES ON TABLE public.execution_tca_symbol_agg_v1 FROM torghut_app;"
    )
    op.execute(
        "REVOKE ALL PRIVILEGES ON TABLE public.execution_tca_strategy_agg_v1 FROM torghut_app;"
    )
    op.execute(
        "REVOKE ALL PRIVILEGES ON TABLE public.execution_tca_metrics FROM torghut_app;"
    )
    ...
    if inspector.has_table("execution_tca_metrics"):
        index_names = {
            index["name"] for index in inspector.get_indexes("execution_tca_metrics")
        }
        if "ix_execution_tca_metrics_computed_at" in index_names:
            op.drop_index(
                "ix_execution_tca_metrics_computed_at",
                table_name="execution_tca_metrics",
            )
        if "ix_execution_tca_metrics_symbol" in index_names:
            op.drop_index(
                "ix_execution_tca_metrics_symbol", table_name="execution_tca_metrics"
            )
