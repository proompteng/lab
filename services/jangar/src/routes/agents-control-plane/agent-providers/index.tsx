...
function AgentProvidersListPage() {
  ...
  const load = React.useCallback(async (value: string, selector?: string) => {
    ...
    setStatus(null)
    try {
      const result = await fetchPrimitiveList({ kind: 'AgentProvider', namespace: value, labelSelector: selector })
      if (!result.ok) {
        setItems([])
        setTotal(0)
      ...
      setStatus(result.items.length === 0 ? 'No agent providers found.' : `Loaded ${result.items.length} providers.`)
    } catch (err) {
      setItems([])
      setTotal(0)
    ...
    <main className="mx-auto w-full max-w-6xl space-y-6 p-6">
      <header className="flex flex-wrap items-start justify-between gap-3">
        <div className="space-y-2">
          ...
          <h1 className="text-lg font-semibold">Agent providers</h1>
          <p className="text-xs text-muted-foreground">Runtime and provider configuration resources.</p>
        </div>
      ...
      {items.length === 0 && !isLoading ? (
        <div className="rounded-none border border-border bg-card p-6 text-xs text-muted-foreground">
          No agent providers found in this namespace.
        </div>
      ) : (
          ...
          {items.map((resource) => {
            ...
            return (
              <li key={`${resourceNamespace}/${name}`} className="border-b border-border last:border-b-0">
                <Link
                  to="/agents-control-plane/agent-providers/$name"
                  params={{ name }}
