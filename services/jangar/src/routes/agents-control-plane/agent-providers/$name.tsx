...
function AgentProviderDetailPage() {
  ...
  const load = React.useCallback(async () => {
    ...
    try {
      ...
      setResource(null)
      setEvents([])
      setError(err instanceof Error ? err.message : 'Failed to load agent provider')
    } finally {
      setIsLoading(false)
    }
    ...
    <main className="mx-auto w-full max-w-6xl space-y-6 p-6">
      <header className="flex flex-wrap items-start justify-between gap-3">
        <div className="space-y-2">
          ...
          <p className="text-xs text-muted-foreground">Agent provider configuration and status.</p>
        </div>
