...
function ImplementationSpecsListPage() {
  ...
  const load = React.useCallback(async (value: string, selector?: string) => {
    ...
    setStatus(null)
    try {
      const result = await fetchPrimitiveList({ kind: 'ImplementationSpec', namespace: value, labelSelector: selector })
      if (!result.ok) {
        setItems([])
        setTotal(0)
    ...
    <main className="mx-auto w-full max-w-6xl space-y-6 p-6">
      <header className="flex flex-wrap items-start justify-between gap-3">
        <div className="space-y-2">
          ...
          <p className="text-xs text-muted-foreground">Normalized implementation definitions.</p>
        </div>
