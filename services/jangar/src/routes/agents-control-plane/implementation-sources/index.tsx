...
function ImplementationSourcesListPage() {
  ...
  const load = React.useCallback(async (value: string) => {
    ...
    setStatus(null)
    try {
      const result = await fetchPrimitiveList({ kind: 'ImplementationSource', namespace: value })
      if (!result.ok) {
        setItems([])
        setTotal(0)
      ...
      setStatus(
        result.items.length === 0 ? 'No implementation sources found.' : `Loaded ${result.items.length} sources.`,
      )
    } catch (err) {
      setItems([])
      setTotal(0)
      setError(err instanceof Error ? err.message : 'Failed to load implementation sources')
    } finally {
      setIsLoading(false)
    }
  }, [])
    ...
    <main className="mx-auto w-full max-w-6xl space-y-6 p-6">
      <header className="flex flex-wrap items-start justify-between gap-3">
        <div className="space-y-2">
          ...
          <h1 className="text-lg font-semibold">Implementation sources</h1>
          <p className="text-xs text-muted-foreground">Upstream feeds that generate implementation specs.</p>
        </div>
      ...
      {items.length === 0 && !isLoading ? (
        <div className="rounded-none border border-border bg-card p-6 text-xs text-muted-foreground">
          No implementation sources found in this namespace.
        </div>
      ) : (
          ...
          {items.map((resource) => {
            ...
            return (
              <li key={`${resourceNamespace}/${name}`} className="border-b border-border last:border-b-0">
                <Link
                  to="/agents-control-plane/implementation-sources/$name"
                  params={{ name }}
