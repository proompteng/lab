import { createFileRoute } from '@tanstack/react-router'

import { proxyAgentsServiceRequest } from '~/server/agents-service-proxy'

const AGENTS_SERVICE_PATH = '/api/agents/control-plane/events'

export const Route = createFileRoute('/api/agents/control-plane/events')({
  server: {
    handlers: {
      GET: async ({ request }: JangarServerRouteArgs) => getAgentsControlPlaneEvents(request),
    },
  },
})

export const getAgentsControlPlaneEvents = (request: Request) => proxyAgentsServiceRequest(request, AGENTS_SERVICE_PATH)
