...
type PrimitiveListPageProps = {
  ...
  onNavigate: (next: NamespaceSearchState) => void
  sectionLabel?: string
}
...
export function PrimitiveListPage({
  ...
  const load = React.useCallback(
    async (value: string, selector?: string) => {
      setIsLoading(true)
      setError(null)
      setStatus(null)
      try {
        const result = await fetchPrimitiveList({ kind, namespace: value, labelSelector: selector })
        if (!result.ok) {
          setItems([])
          setTotal(0)
