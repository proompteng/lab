...
export const SCHEDULE_RUNNER_AGENTS_SERVICE_BASE_URL_ENV = 'AGENTS_SERVICE_BASE_URL'
...
export const buildScheduleRunnerCommand = (): string =>
  [
    ...
    `  const agentsServiceBaseUrlEnv = ${JSON.stringify(SCHEDULE_RUNNER_AGENTS_SERVICE_BASE_URL_ENV)};`,
    `  const defaultAgentsServiceBaseUrl = ${JSON.stringify(DEFAULT_AGENTS_SERVICE_BASE_URL)};`,
    ...
    '  };',
    '  const targetKind = String(manifest.kind || "");',
    '  if (!["AgentRun", "OrchestrationRun"].includes(targetKind)) throw new Error(`unsupported schedule target kind: ${targetKind || "unknown"}`);',
    ...
    '    }',
    '  }',
    '  const agentsServiceBaseUrl = (readEnv(agentsServiceBaseUrlEnv) || defaultAgentsServiceBaseUrl).replace(/\\/+$/, "");',
    '  const requestAgentsJson = async (path, options = {}) => {',
    '    const url = new URL(path, `${agentsServiceBaseUrl}/`);',
    '    const searchParams = asRecord(options.searchParams) ?? {};',
    ...
    '      headers: { accept: "application/json", "x-agents-client": "jangar-schedule-runner", ...(asRecord(options.headers) ?? {}) },',
    '      body: options.body,',
    '    });',
    '    if (!response.ok) {',
    '      const body = (await response.text()).slice(0, 1000);',
    '      throw new Error(`schedule runner Agents service request failed: ${response.status} ${body}`);',
    '    }',
    ...
    '    const labelSelector = `${swarmNameLabelName}=${swarmName},${stageLabelName}=${stage}`;',
    '    const listPayload = await requestAgentsJson("/api/agents/control-plane/resources", {',
    '      searchParams: { kind: targetKind, namespace, labelSelector },',
    '    });',
    ...
    '  const deliveryId = asString(readNested(manifest, ["metadata", "labels", "agents.proompteng.ai/delivery-id"])) || asString(readNested(manifest, ["metadata", "name"])) || randomUUID();',
    '  await requestAgentsJson("/api/agents/control-plane/resource", {',
    '    method: "POST",',
