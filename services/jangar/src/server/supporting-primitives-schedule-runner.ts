...
export const buildScheduleRunnerCommand = (): string =>
  [
    ...
    '    }',
    '  };',
    '  const assertStampedAdmissionValue = (stampedValue, currentValue, subject, missingMessage) => {',
    '    if (!currentValue) throw new Error(`current schedule ${subject} is missing`);',
    '    if (!stampedValue) throw new Error(missingMessage);',
    '    if (stampedValue !== currentValue) throw new Error(`stale schedule ${subject} ${stampedValue}; current is ${currentValue}`);',
    '  };',
    ...
    '    assertStampedAdmissionValue(stampedPassportId, currentPassportId, "admission passport", `missing schedule admission passport annotation for ${consumer}`);',
    '    assertStampedAdmissionValue(stampedRuntimeDigest, currentRuntimeDigest, "runtime-kit digest", `missing schedule runtime-kit digest annotation for ${consumer}`);',
    '    assertStampedAdmissionValue(stampedRecoveryDigest, currentRecoveryDigest, "recovery-case digest", `missing schedule recovery-case digest annotation for ${consumer}`);',
    '    assertStampedAdmissionValue(stampedRuntimeKits, currentRuntimeKits, "required runtime kits", `missing schedule required runtime kits annotation for ${consumer}`);',
