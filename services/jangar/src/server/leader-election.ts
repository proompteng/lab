...
import { isRuntimeTestEnv, resolveLeaderElectionSettings } from './control-plane-config'
...
const ensureMetrics = () => {
  ...
  const changesCounter = meter.createCounter('jangar_leader_changes_total', {
    description: 'Count of Jangar leader election transitions (leader<->follower).',
  })
...
export const stopLeaderElectionRuntime = () => {
  globalState.__jangarLeaderElection?.stop?.()
