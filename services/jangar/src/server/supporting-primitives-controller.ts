...
const buildScheduleRunnerCommand = (): string =>
  [
    ...
    '    if (currentPassportId !== stampedPassportId) {',
    '      throw new Error(`stale schedule admission passport: stamped=${stampedPassportId} current=${currentPassportId || "missing"}`);',
    '    }',
    '    const decision = asString(passport.decision);',
    '    if (decision !== "allow") {',
    '      throw new Error(`current schedule admission passport ${stampedPassportId} is ${decision || "missing"}`);',
    '    }',
    '    assertFreshUntil(passport.fresh_until, `current schedule admission passport ${stampedPassportId}`);',
    '    const stampedRuntimeDigest = asString(annotations[admissionAnnotations.runtimeDigest]);',
    '    const currentRuntimeDigest = asString(passport.runtime_kit_set_digest);',
    '    if (stampedRuntimeDigest && stampedRuntimeDigest !== currentRuntimeDigest) {',
    '      throw new Error(`stale schedule runtime-kit digest: stamped=${stampedRuntimeDigest} current=${currentRuntimeDigest || "missing"}`);',
    '    }',
    '    const stampedRecoveryDigest = asString(annotations[admissionAnnotations.recoveryDigest]);',
    '    const currentRecoveryDigest = asString(passport.recovery_case_set_digest);',
    '    if (stampedRecoveryDigest && stampedRecoveryDigest !== currentRecoveryDigest) {',
    '      throw new Error(`stale schedule recovery-case digest: stamped=${stampedRecoveryDigest} current=${currentRecoveryDigest || "missing"}`);',
    '    }',
    '    const stampedRuntimeKits = normalizeRuntimeKits(annotations[admissionAnnotations.runtimeKits]);',
    '    const currentRuntimeKits = normalizeRuntimeKits(passport.required_runtime_kits);',
    '    if (stampedRuntimeKits && stampedRuntimeKits !== currentRuntimeKits) {',
    '      throw new Error(`stale schedule required runtime kits: stamped=${stampedRuntimeKits} current=${currentRuntimeKits || "missing"}`);',
    '    }',
                ...
                containers: [
                  {
                    ...
                    env: [
                      ...
                      },
                      {
                        ...
                        value: supportingConfig.scheduleRunnerAdmissionStatusUrl,
                      },
                      {
