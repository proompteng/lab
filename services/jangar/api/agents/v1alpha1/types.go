...
// +kubebuilder:validation:XValidation:rule="!has(self.systemPrompt) && !has(self.systemPromptRef)",message="AgentRun-level systemPrompt/systemPromptRef overrides are not allowed; configure Agent.spec.defaults instead"
	// +kubebuilder:validation:XValidation:rule="!has(self.parameters) || self.parameters.all(key, key != 'prompt')",message="spec.parameters.prompt is not allowed; use ImplementationSpec.spec.text"
...
type WorkflowStep struct {
	...
	Implementation        *InlineImplementation `json:"implementation,omitempty"`
	// +kubebuilder:validation:MaxProperties=100
	// +kubebuilder:validation:XValidation:rule="!has(self.parameters) || self.parameters.all(key, key != 'prompt')",message="workflow step parameters.prompt is not allowed; use ImplementationSpec.spec.text"
	Parameters          map[string]string `json:"parameters,omitempty"`
	Workload            *WorkloadSpec     `json:"workload,omitempty"`
