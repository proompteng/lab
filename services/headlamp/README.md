...
This directory maintains the pinned upstream source and local patch set for the repo-owned Headlamp image used by
`argocd/applications/headlamp`. Production images are built by the `headlamp-image` Nix derivation in
...
This build also applies a local upstream patch set from `patches/` that changes the
