...
export function GatewayFrame({
  ...
  return (
    <main id="main-content" className="flex h-screen min-h-[720px] flex-col bg-zinc-950 text-zinc-100">
      <header className="flex h-12 shrink-0 items-center justify-between border-b border-zinc-900 px-5">
        <div className="flex min-w-0 items-center gap-4">
          <a href="/" className="flex min-w-0 items-center gap-3">
            <div className="flex size-7 items-center justify-center rounded-md border border-zinc-800 bg-zinc-950">
              <ShieldCheck className="size-4 text-zinc-300" aria-hidden="true" />
            </div>
            <h1 className="truncate text-sm font-semibold text-zinc-50">SAG</h1>
          </a>
        ...
        <div className="flex items-center gap-3 text-xs text-zinc-500">
          <span>{snapshot.stats.totalTasks} tasks</span>
          <Separator orientation="vertical" className="h-4" />
          <span>{snapshot.stats.totalEvents} events</span>
          <Separator orientation="vertical" className="h-4" />
          <span>{snapshot.stats.awaitingApproval} pending</span>
          <a
