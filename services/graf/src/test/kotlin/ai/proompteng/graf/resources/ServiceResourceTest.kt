...
class ServiceResourceTest {
  @Test
  fun `root responds with Graf metadata from env`() = withEnv(
    mapOf(
  ...
  @Test
  fun `healthz returns ok status and port`() = withEnv(
    mapOf(
      "PORT" to "8080",
    ),
  ) {
  ...
  private fun withEnv(env: Map<String, String?>, block: () -> Unit) {
