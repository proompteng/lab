...
class GraphResource @Inject constructor(
  ...
  suspend fun upsertRelationships(payload: RelationshipBatchRequest): BatchResponse =
    graphService.upsertRelationships(payload)

  @PATCH
  @Path("/entities/{id}")
  @GrafRouteTemplate("PATCH /v1/entities/{id}")
  suspend fun patchEntity(@PathParam("id") id: String, payload: EntityPatchRequest): GraphResponse =
    graphService.patchEntity(id, payload)

  @PATCH
  @Path("/relationships/{id}")
  @GrafRouteTemplate("PATCH /v1/relationships/{id}")
  suspend fun patchRelationship(@PathParam("id") id: String, payload: RelationshipPatchRequest): GraphResponse =
    graphService.patchRelationship(id, payload)

  @DELETE
  @Path("/entities/{id}")
  @GrafRouteTemplate("DELETE /v1/entities/{id}")
  suspend fun deleteEntity(@PathParam("id") id: String, payload: DeleteRequest): GraphResponse =
    graphService.deleteEntity(id, payload)

  @DELETE
  @Path("/relationships/{id}")
  @GrafRouteTemplate("DELETE /v1/relationships/{id}")
  suspend fun deleteRelationship(@PathParam("id") id: String, payload: DeleteRequest): GraphResponse =
    graphService.deleteRelationship(id, payload)
