...
export default Effect.gen(function* () {
  ...
  yield* sql`
    ...
    DROP CONSTRAINT lifecycle_commands_not_due_reason_check,
    ADD CONSTRAINT lifecycle_commands_not_due_reason_check CHECK (
      not_due_reason IN ('MONTH_END_CADENCE', 'STALE_PAPER_BOOTSTRAP', 'STALE_CAPITAL_BOOTSTRAP')
    )
  `
          ...
          AND (
            (
              ...
              AND (
                ...
                OR (
                  state.reason IN (
                    'execution episode restricted effective authority: flat exact receipt finalized',
