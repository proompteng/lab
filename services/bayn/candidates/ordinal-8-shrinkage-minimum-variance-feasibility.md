...
The cleanup commit is deliberately a direct child of proof commit
`78c485444fc11a1a148c2aeb5fc6a050497a9f61`. Its tree removes both executable proof files while retaining this record.
The closed pull-request branch and GitHub pull-request head ref both point to that cleanup history, so a normal fetch of
either ref transfers the proof commit and its source and test blobs. Reachability is verified with
`git merge-base --is-ancestor 78c485444fc11a1a148c2aeb5fc6a050497a9f61 HEAD` before the branch is published.
