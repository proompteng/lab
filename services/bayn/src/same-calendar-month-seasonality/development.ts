...
import {
  ...
  CANDIDATE_12_HOLDOUT_END,
  CANDIDATE_12_HOLDOUT_START,
  CANDIDATE_12_SNAPSHOT_ID,
  candidate12BehaviorMaterial,
  ...
  type Candidate12DevelopmentReport,
  type Candidate12Failure,
  type Candidate12Plan,
  type Candidate12PreparedData,
  type Candidate12Registration,
  type Candidate12SpecificationReport,
  type Candidate12Symbol,
} from './model'
import { buildCandidate12Plan, candidate12TerminalLiquidationIsComplete } from './strategy'
...
const canonicalHash = (operation: string, material: unknown): Result.Result<string, Candidate12Failure> =>
  pipe(
    canonicalHashV1Result(material),
    Result.mapError((cause): Candidate12Failure => ({ _tag: 'Candidate12HashFailure', operation, cause })),
  )
...
export const candidate12DatasetHashes = (
  sessions: readonly IsoDate[],
  bars: Candidate12Dataset['bars'],
): Result.Result<{ readonly sessionsContentHash: string; readonly barsContentHash: string }, Candidate12Failure> =>
  Result.all({
    sessionsContentHash: canonicalHash('development-sessions', {
      schemaVersion: 'bayn.candidate-12-development-sessions.v1',
      snapshotId: CANDIDATE_12_SNAPSHOT_ID,
      sessions,
    }),
    barsContentHash: canonicalHash('development-bars', {
      schemaVersion: 'bayn.candidate-12-development-bars.v1',
      snapshotId: CANDIDATE_12_SNAPSHOT_ID,
...
const validDatasetBar = (bar: Candidate12Dataset['bars'][number]): boolean =>
...
export const prepareCandidate12DevelopmentData = (
  dataset: Candidate12Dataset,
): Result.Result<Candidate12PreparedData, Candidate12Failure> => {
  const expectedSessions = candidate12DevelopmentSessions()
  if (dataset.snapshotId !== CANDIDATE_12_SNAPSHOT_ID) {
    return fail('dataset', `snapshot ${dataset.snapshotId} differs from ${CANDIDATE_12_SNAPSHOT_ID}`)
  }
  if (!exactDates(dataset.sessions, expectedSessions)) return fail('dataset', 'official development sessions differ')
  const expectedBarCount = expectedSessions.length * candidate12Universe.length
  if (dataset.bars.length !== expectedBarCount) {
    return fail('dataset', `expected ${expectedBarCount} bars, observed ${dataset.bars.length}`)
  }
  return pipe(
    candidate12DatasetHashes(dataset.sessions, dataset.bars),
    Result.flatMap((hashes) => {
      ...
      for (let sessionIndex = 0; sessionIndex < expectedSessions.length; sessionIndex += 1) {
        ...
        const bars: Partial<Record<Candidate12Symbol, AlignedSession['bars'][string]>> = {}
        for (let symbolIndex = 0; symbolIndex < candidate12Universe.length; symbolIndex += 1) {
          const symbol = candidate12Universe.at(symbolIndex)
          const bar = dataset.bars.at(sessionIndex * candidate12Universe.length + symbolIndex)
        ...
        pipe(
          ...
          Result.mapError(
            (cause): Candidate12Failure => ({
              _tag: 'Candidate12SimulationFailure',
...
const runSimulation = (
  ...
  runId: string,
): Result.Result<SimulationResult, Candidate12Failure> => {
  ...
  if (Result.isFailure(raw)) {
    return Result.fail({ _tag: 'Candidate12SimulationFailure', simulation, cause: raw.failure })
  }
  ...
  if (Result.isFailure(metrics)) {
    return Result.fail({ _tag: 'Candidate12SimulationFailure', simulation, cause: metrics.failure })
  }
...
const performanceByDate = (
  ...
  name: string,
): Result.Result<ReadonlyMap<IsoDate, DailyPerformancePoint>, Candidate12Failure> => {
...
): Result.Result<QualificationSeries, Candidate12Failure> =>
  pipe(
    Result.all({
...
interface Candidate12BenchmarkResults {
...
const evaluateSpecification = (
  prepared: Candidate12PreparedData,
  plan: Candidate12Plan,
  familyStrategyHash: string,
  familyRunId: string,
  benchmarks: Candidate12BenchmarkResults,
): Result.Result<Candidate12SpecificationReport, Candidate12Failure> =>
  pipe(
    canonicalHash('specification-strategy', {
      schemaVersion: 'bayn.candidate-12-specification-strategy.v1',
      familyStrategyHash,
    ...
    Result.flatMap((strategyHash) =>
      pipe(
        canonicalHash('specification-run', {
          schemaVersion: 'bayn.candidate-12-specification-run.v1',
          familyRunId,
          strategyHash,
        }),
          ...
          pipe(
            Result.all({
              ...
              doubleCostStrategy: runSimulation(
                ...
                plan.evaluationStartIndex,
                candidate12SimulationProtocol,
                BigInt(candidate12SimulationProtocol.executionModel.doubleCostMultiplier) * MICROS,
                runId,
              ),
            }),
              ...
              return pipe(
                ...
                Result.flatMap((series) =>
                  pipe(
                    ...
                    Result.mapError(
                      (cause): Candidate12Failure => ({ _tag: 'Candidate12QualificationFailure', cause }),
                    ),
                    Result.map((analysis): Candidate12SpecificationReport => {
...
export interface Candidate12SelectionFact {
  readonly specificationId: Candidate12SpecificationReport['specification']['id']
...
export const selectCandidate12SpecificationId = (
  facts: readonly Candidate12SelectionFact[],
): Candidate12SelectionFact['specificationId'] | null =>
  facts
...
export const selectCandidate12Specification = (
  reports: readonly Candidate12SpecificationReport[],
): Candidate12SpecificationReport | undefined => {
  const selectedId = selectCandidate12SpecificationId(
    reports.map((report) => ({
...
export const evaluateCandidate12Development = (
  registration: Candidate12Registration,
  dataset: Candidate12Dataset,
  preflight: CandidateDevelopmentPreflightPass,
): Result.Result<Candidate12DevelopmentReport, Candidate12Failure> => {
  const prepared = prepareCandidate12DevelopmentData(dataset)
  if (Result.isFailure(prepared)) return Result.fail(prepared.failure)
  const parameterHash = canonicalHash('parameters', {
    strategy: candidate12Protocol,
    simulation: candidate12SimulationProtocol,
    statistics: candidate12DevelopmentStatisticsPolicy,
    selectionMultiplicity: candidate12Specifications.length,
    priorAttemptIds: candidate12PriorAttemptIds,
  })
  if (Result.isFailure(parameterHash)) return Result.fail(parameterHash.failure)
  const behaviorHash = canonicalHash('behavior', candidate12BehaviorMaterial)
  if (Result.isFailure(behaviorHash)) return Result.fail(behaviorHash.failure)
  const terminalCash = candidate12TerminalLiquidationIsComplete()
  ...
  const familyStrategyHash = canonicalHash('family-strategy', {
    schemaVersion: 'bayn.candidate-12-family-strategy.v1',
  ...
  const familyRunId = canonicalHash('development-run', {
    schemaVersion: 'bayn.candidate-12-development-run.v1',
    ...
    developmentStart: CANDIDATE_12_DEVELOPMENT_START,
    developmentEnd: CANDIDATE_12_DEVELOPMENT_END,
  ...
  const plans = Result.all(
    candidate12Specifications.map((specification) =>
      buildCandidate12Plan(prepared.success.sessions, preflight, specification),
    ),
  )
  ...
  const benchmarkRunId = canonicalHash('benchmark-run', {
    schemaVersion: 'bayn.candidate-12-benchmark-run.v1',
  ...
  const benchmarks: Candidate12BenchmarkResults = {
  ...
  const selected = selectCandidate12Specification(specificationReports.success)
  ...
  const reportMaterial = {
    schemaVersion: 'bayn.candidate-12-development-report.v1' as const,
    ...
    dataset: {
      snapshotId: dataset.snapshotId,
      firstSession: prepared.success.sessions.at(0)?.date ?? CANDIDATE_12_DEVELOPMENT_START,
      lastSession: prepared.success.sessions.at(-1)?.date ?? CANDIDATE_12_DEVELOPMENT_END,
    ...
    geometry: preflight,
    selection: {
      specificationCount: candidate12Specifications.length,
      familyMultiplicityDivisor: candidate12Specifications.length,
      priorAttemptCount: candidate12PriorAttemptIds.length,
    ...
    holdout: {
      start: CANDIDATE_12_HOLDOUT_START,
      end: CANDIDATE_12_HOLDOUT_END,
