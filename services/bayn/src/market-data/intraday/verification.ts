...
export const reverifyIntradayMarketSnapshot = (
  ...
  Result.gen(function* () {
    ...
    const latestHashes = yield* Result.all({
      expected: hash(verified.latestQuotes, 'reverified latest quotes'),
      actual: hash(snapshot.latestQuotes, 'bound latest quotes'),
    })
    if (verified.manifest.snapshotId !== manifest.snapshotId || latestHashes.expected !== latestHashes.actual) {
