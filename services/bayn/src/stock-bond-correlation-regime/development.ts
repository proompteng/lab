...
import {
  ...
  CANDIDATE_15_HOLDOUT_END,
  CANDIDATE_15_HOLDOUT_START,
  CANDIDATE_15_PROTOCOL_HASH,
  CANDIDATE_15_SNAPSHOT_ID,
  candidate15BehaviorMaterial,
  candidate15DevelopmentSessions,
  candidate15DevelopmentStatisticsPolicy,
  candidate15PriorAttemptIds,
  candidate15Protocol,
  candidate15SelectionMultiplicity,
  candidate15SimulationProtocol,
  candidate15Specifications,
  candidate15Universe,
  type Candidate15CostReplay,
  type Candidate15Dataset,
  type Candidate15DevelopmentEvaluation,
  type Candidate15DevelopmentReport,
  type Candidate15Failure,
  type Candidate15Plan,
  type Candidate15PreparedData,
  type Candidate15Registration,
  type Candidate15SpecificationReport,
  type Candidate15Symbol,
} from './model'
import { buildCandidate15Plan, candidate15TerminalLiquidationIsComplete } from './strategy'
...
const canonicalHash = (operation: string, material: unknown): Result.Result<string, Candidate15Failure> =>
  pipe(
    canonicalHashV1Result(material),
    Result.mapError((cause): Candidate15Failure => ({ _tag: 'Candidate15HashFailure', operation, cause })),
  )
...
export const candidate15DatasetHashes = (
  sessions: readonly IsoDate[],
  bars: Candidate15Dataset['bars'],
): Result.Result<{ readonly sessionsContentHash: string; readonly barsContentHash: string }, Candidate15Failure> =>
  Result.all({
    sessionsContentHash: canonicalHash('development-sessions', {
      schemaVersion: 'bayn.candidate-15-development-sessions.v1',
      snapshotId: CANDIDATE_15_SNAPSHOT_ID,
      sessions,
    }),
    barsContentHash: canonicalHash('development-bars', {
      schemaVersion: 'bayn.candidate-15-development-bars.v1',
      snapshotId: CANDIDATE_15_SNAPSHOT_ID,
...
const validDatasetBar = (bar: Candidate15Dataset['bars'][number]): boolean =>
...
export const prepareCandidate15DevelopmentData = (
  dataset: Candidate15Dataset,
): Result.Result<Candidate15PreparedData, Candidate15Failure> => {
  const expectedSessions = candidate15DevelopmentSessions()
  if (dataset.snapshotId !== CANDIDATE_15_SNAPSHOT_ID) {
    return fail('dataset', `snapshot ${dataset.snapshotId} differs from ${CANDIDATE_15_SNAPSHOT_ID}`)
  }
  if (!exactDates(dataset.sessions, expectedSessions)) return fail('dataset', 'official development sessions differ')
  const expectedBarCount = expectedSessions.length * candidate15Universe.length
  if (dataset.bars.length !== expectedBarCount) {
    return fail('dataset', `expected ${expectedBarCount} bars, observed ${dataset.bars.length}`)
  }
  return pipe(
    candidate15DatasetHashes(dataset.sessions, dataset.bars),
    Result.flatMap((hashes) => {
      ...
      for (let sessionIndex = 0; sessionIndex < expectedSessions.length; sessionIndex += 1) {
        ...
        const bars: Partial<Record<Candidate15Symbol, AlignedSession['bars'][string]>> = {}
        for (let symbolIndex = 0; symbolIndex < candidate15Universe.length; symbolIndex += 1) {
          const symbol = candidate15Universe.at(symbolIndex)
          const bar = dataset.bars.at(sessionIndex * candidate15Universe.length + symbolIndex)
        ...
        pipe(
          ...
          Result.mapError(
            (cause): Candidate15Failure => ({
              _tag: 'Candidate15SimulationFailure',
...
const selectEvaluationWindow = (
...
): Result.Result<SimulationResult, Candidate15Failure> => {
  ...
  if (Result.isFailure(metrics)) {
    return Result.fail({ _tag: 'Candidate15SimulationFailure', simulation, cause: metrics.failure })
  }
...
const runSimulation = (
  ...
  if (Result.isFailure(raw)) {
    return Result.fail({ _tag: 'Candidate15SimulationFailure', simulation, cause: raw.failure })
  }
...
interface ReplayState {
  ...
  readonly positions: Readonly<Record<Candidate15Symbol, ReplayPosition>>
...
const initialReplayPositions = (): Readonly<Record<Candidate15Symbol, ReplayPosition>> =>
  Object.fromEntries(
    candidate15Universe.map((symbol) => [symbol, { quantityMicros: 0n, costBasisMicros: 0n }]),
  ) as Readonly<Record<Candidate15Symbol, ReplayPosition>>
...
const position = (
  positions: Readonly<Record<Candidate15Symbol, ReplayPosition>>,
  symbol: string,
): Result.Result<{ readonly symbol: Candidate15Symbol; readonly value: ReplayPosition }, Candidate15Failure> => {
  if (!candidate15Universe.includes(symbol as Candidate15Symbol)) {
    return fail('double-cost-replay', `order symbol ${symbol} is outside the frozen universe`)
  }
  const candidateSymbol = symbol as Candidate15Symbol
  return Result.succeed({ symbol: candidateSymbol, value: positions[candidateSymbol] })
}

const updatePosition = (
  positions: Readonly<Record<Candidate15Symbol, ReplayPosition>>,
  symbol: Candidate15Symbol,
  value: ReplayPosition,
): Readonly<Record<Candidate15Symbol, ReplayPosition>> => ({ ...positions, [symbol]: value })

const orderQuantity = (order: SimulatedOrder): Result.Result<bigint, Candidate15Failure> =>
...
const applyReplayOrders = (
  ...
  costMultiplierMicros: bigint,
  runId: string,
): Result.Result<ReplayState, Candidate15Failure> => {
  let next = state
  ...
  if (Result.isFailure(fees)) {
    return Result.fail({ _tag: 'Candidate15SimulationFailure', simulation: 'double-cost-replay', cause: fees.failure })
  }
  if (fees.success.totalMicros > 0n) {
    const fee = makeFeeEvent(runId, session.date, fees.success)
    if (Result.isFailure(fee)) {
      return Result.fail({ _tag: 'Candidate15SimulationFailure', simulation: 'double-cost-replay', cause: fee.failure })
    }
  ...
  return next.cashMicros < 0n
    ? Result.fail({
        _tag: 'Candidate15DoubledCostReplayInvalid',
...
export const replayCandidate15FixedOrderCosts = (
  sessions: readonly AlignedSession[],
  ...
  costMultiplierMicros: bigint,
  runId: string,
): Result.Result<Candidate15CostReplay, Candidate15Failure> => {
  ...
  const terminalCash = candidate15Universe.every((symbol) => state.positions[symbol].quantityMicros === 0n)
  return Result.succeed({
...
const performanceByDate = (
  ...
  name: string,
): Result.Result<ReadonlyMap<IsoDate, DailyPerformancePoint>, Candidate15Failure> => {
...
): Result.Result<QualificationSeries, Candidate15Failure> =>
  pipe(
    Result.all({
...
interface Candidate15BenchmarkResults {
...
interface EvaluatedCandidate15Specification {
  readonly report: Candidate15SpecificationReport
...
const evaluateSpecification = (
  prepared: Candidate15PreparedData,
  plan: Candidate15Plan,
  familyStrategyHash: string,
  familyRunId: string,
  benchmarks: Candidate15BenchmarkResults,
): Result.Result<EvaluatedCandidate15Specification, Candidate15Failure> => {
  const strategyHash = canonicalHash('specification-strategy', {
    schemaVersion: 'bayn.candidate-15-specification-strategy.v1',
    familyStrategyHash,
  ...
  const runId = canonicalHash('specification-run', {
    schemaVersion: 'bayn.candidate-15-specification-run.v1',
    familyRunId,
  ...
  const stressed = replayCandidate15FixedOrderCosts(
    prepared.sessions,
    baselineWithTrace,
    plan.simulationStartIndex,
    plan.evaluationStartIndex,
  ...
  if (Result.isFailure(analysisResult)) {
    return Result.fail({ _tag: 'Candidate15QualificationFailure', cause: analysisResult.failure })
  }
  ...
  const report: Candidate15SpecificationReport = {
    specification: plan.specification,
...
export const evaluateCandidate15Development = (
  registration: Candidate15Registration,
  dataset: Candidate15Dataset,
  preflight: CandidateDevelopmentPreflightPass,
): Result.Result<Candidate15DevelopmentEvaluation, Candidate15Failure> => {
  if (preflight.protocolIdentity.protocolHash !== CANDIDATE_15_PROTOCOL_HASH) {
    return fail('preflight', `protocol hash ${preflight.protocolIdentity.protocolHash} differs from Candidate 15`)
  }
  const prepared = prepareCandidate15DevelopmentData(dataset)
  if (Result.isFailure(prepared)) return Result.fail(prepared.failure)
  const parameterHash = canonicalHash('parameters', {
    strategy: candidate15Protocol,
    simulation: candidate15SimulationProtocol,
    statistics: candidate15DevelopmentStatisticsPolicy,
    selectionMultiplicity: candidate15SelectionMultiplicity,
    priorAttemptIds: candidate15PriorAttemptIds,
  ...
  const behaviorHash = canonicalHash('behavior', candidate15BehaviorMaterial)
  if (Result.isFailure(behaviorHash)) return Result.fail(behaviorHash.failure)
  const terminalCash = candidate15TerminalLiquidationIsComplete()
  ...
  const familyStrategyHash = canonicalHash('family-strategy', {
    schemaVersion: 'bayn.candidate-15-family-strategy.v1',
  ...
  const familyRunId = canonicalHash('development-run', {
    schemaVersion: 'bayn.candidate-15-development-run.v1',
    ...
    developmentStart: CANDIDATE_15_DEVELOPMENT_START,
    developmentEnd: CANDIDATE_15_DEVELOPMENT_END,
  ...
  const plans = Result.all(
    candidate15Specifications.map((specification) =>
      buildCandidate15Plan(prepared.success.sessions, preflight, specification),
    ),
  )
  ...
  const benchmarkRunId = canonicalHash('benchmark-run', {
    schemaVersion: 'bayn.candidate-15-benchmark-run.v1',
  ...
  const benchmarks: Candidate15BenchmarkResults = {
  ...
  const reportMaterial = {
    schemaVersion: 'bayn.candidate-15-development-report.v1' as const,
    ...
    dataset: {
      snapshotId: dataset.snapshotId,
      firstSession: prepared.success.sessions.at(0)?.date ?? CANDIDATE_15_DEVELOPMENT_START,
      lastSession: prepared.success.sessions.at(-1)?.date ?? CANDIDATE_15_DEVELOPMENT_END,
    ...
    geometry: preflight,
    selection: {
      specificationCount: candidate15Specifications.length,
      familyMultiplicityDivisor: candidate15SelectionMultiplicity,
      priorAttemptCount: candidate15PriorAttemptIds.length,
    ...
    holdout: {
      start: CANDIDATE_15_HOLDOUT_START,
      end: CANDIDATE_15_HOLDOUT_END,
  ...
  const report: Candidate15DevelopmentReport = {
    ...reportMaterial,
