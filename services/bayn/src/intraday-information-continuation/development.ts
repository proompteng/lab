...
import {
  ...
  CANDIDATE_14_HOLDOUT_END,
  CANDIDATE_14_HOLDOUT_START,
  CANDIDATE_14_PROTOCOL_HASH,
  CANDIDATE_14_SNAPSHOT_ID,
  candidate14BehaviorMaterial,
  candidate14DevelopmentSessions,
  candidate14DevelopmentStatisticsPolicy,
  candidate14PriorAttemptIds,
  candidate14Protocol,
  candidate14SelectionMultiplicity,
  candidate14SimulationProtocol,
  candidate14Specifications,
  candidate14Universe,
  type Candidate14CostReplay,
  type Candidate14Dataset,
  type Candidate14DevelopmentEvaluation,
  type Candidate14DevelopmentReport,
  type Candidate14Failure,
  type Candidate14Plan,
  type Candidate14PreparedData,
  type Candidate14Registration,
  type Candidate14SpecificationReport,
  type Candidate14Symbol,
} from './model'
import { buildCandidate14Plan, candidate14TerminalLiquidationIsComplete } from './strategy'
...
const canonicalHash = (operation: string, material: unknown): Result.Result<string, Candidate14Failure> =>
  pipe(
    canonicalHashV1Result(material),
    Result.mapError((cause): Candidate14Failure => ({ _tag: 'Candidate14HashFailure', operation, cause })),
  )
...
export const candidate14DatasetHashes = (
  sessions: readonly IsoDate[],
  bars: Candidate14Dataset['bars'],
): Result.Result<{ readonly sessionsContentHash: string; readonly barsContentHash: string }, Candidate14Failure> =>
  Result.all({
    sessionsContentHash: canonicalHash('development-sessions', {
      schemaVersion: 'bayn.candidate-14-development-sessions.v1',
      snapshotId: CANDIDATE_14_SNAPSHOT_ID,
      sessions,
    }),
    barsContentHash: canonicalHash('development-bars', {
      schemaVersion: 'bayn.candidate-14-development-bars.v1',
      snapshotId: CANDIDATE_14_SNAPSHOT_ID,
...
const validDatasetBar = (bar: Candidate14Dataset['bars'][number]): boolean =>
...
export const prepareCandidate14DevelopmentData = (
  dataset: Candidate14Dataset,
): Result.Result<Candidate14PreparedData, Candidate14Failure> => {
  const expectedSessions = candidate14DevelopmentSessions()
  if (dataset.snapshotId !== CANDIDATE_14_SNAPSHOT_ID) {
    return fail('dataset', `snapshot ${dataset.snapshotId} differs from ${CANDIDATE_14_SNAPSHOT_ID}`)
  }
  if (!exactDates(dataset.sessions, expectedSessions)) return fail('dataset', 'official development sessions differ')
  const expectedBarCount = expectedSessions.length * candidate14Universe.length
  if (dataset.bars.length !== expectedBarCount) {
    return fail('dataset', `expected ${expectedBarCount} bars, observed ${dataset.bars.length}`)
  }
  return pipe(
    candidate14DatasetHashes(dataset.sessions, dataset.bars),
    Result.flatMap((hashes) => {
      ...
      for (let sessionIndex = 0; sessionIndex < expectedSessions.length; sessionIndex += 1) {
        ...
        const bars: Partial<Record<Candidate14Symbol, AlignedSession['bars'][string]>> = {}
        for (let symbolIndex = 0; symbolIndex < candidate14Universe.length; symbolIndex += 1) {
          const symbol = candidate14Universe.at(symbolIndex)
          const bar = dataset.bars.at(sessionIndex * candidate14Universe.length + symbolIndex)
        ...
        pipe(
          ...
          Result.mapError(
            (cause): Candidate14Failure => ({
              _tag: 'Candidate14SimulationFailure',
...
const selectEvaluationWindow = (
...
): Result.Result<SimulationResult, Candidate14Failure> => {
  ...
  if (Result.isFailure(metrics)) {
    return Result.fail({ _tag: 'Candidate14SimulationFailure', simulation, cause: metrics.failure })
  }
...
const runSimulation = (
  ...
  if (Result.isFailure(raw)) {
    return Result.fail({ _tag: 'Candidate14SimulationFailure', simulation, cause: raw.failure })
  }
...
interface ReplayState {
  ...
  readonly positions: Readonly<Record<Candidate14Symbol, ReplayPosition>>
...
const initialReplayPositions = (): Readonly<Record<Candidate14Symbol, ReplayPosition>> =>
  Object.fromEntries(
    candidate14Universe.map((symbol) => [symbol, { quantityMicros: 0n, costBasisMicros: 0n }]),
  ) as Readonly<Record<Candidate14Symbol, ReplayPosition>>
...
const position = (
  positions: Readonly<Record<Candidate14Symbol, ReplayPosition>>,
  symbol: string,
): Result.Result<{ readonly symbol: Candidate14Symbol; readonly value: ReplayPosition }, Candidate14Failure> => {
  if (!candidate14Universe.includes(symbol as Candidate14Symbol)) {
    return fail('double-cost-replay', `order symbol ${symbol} is outside the frozen universe`)
  }
  const candidateSymbol = symbol as Candidate14Symbol
...
): Readonly<Record<Candidate14Symbol, ReplayPosition>> => ({ ...positions, [symbol]: value })

const orderQuantity = (order: SimulatedOrder): Result.Result<bigint, Candidate14Failure> =>
...
const applyReplayOrders = (
  ...
  costMultiplierMicros: bigint,
  runId: string,
): Result.Result<ReplayState, Candidate14Failure> => {
  let next = state
  ...
  if (Result.isFailure(fees)) {
    return Result.fail({ _tag: 'Candidate14SimulationFailure', simulation: 'double-cost-replay', cause: fees.failure })
  }
  if (fees.success.totalMicros > 0n) {
    const fee = makeFeeEvent(runId, session.date, fees.success)
    if (Result.isFailure(fee)) {
      return Result.fail({ _tag: 'Candidate14SimulationFailure', simulation: 'double-cost-replay', cause: fee.failure })
    }
  ...
  return next.cashMicros < 0n
    ? Result.fail({
        _tag: 'Candidate14DoubledCostReplayInvalid',
...
const closeReplaySession = (
  ...
  for (const symbol of candidate14Universe) {
...
export const replayCandidate14FixedOrderCosts = (
  sessions: readonly AlignedSession[],
  ...
  costMultiplierMicros: bigint,
  runId: string,
): Result.Result<Candidate14CostReplay, Candidate14Failure> => {
  ...
  const terminalCash = candidate14Universe.every((symbol) => state.positions[symbol].quantityMicros === 0n)
  return Result.succeed({
...
const performanceByDate = (
  ...
  name: string,
): Result.Result<ReadonlyMap<IsoDate, DailyPerformancePoint>, Candidate14Failure> => {
...
): Result.Result<QualificationSeries, Candidate14Failure> =>
  pipe(
    Result.all({
...
interface Candidate14BenchmarkResults {
...
interface EvaluatedCandidate14Specification {
  readonly report: Candidate14SpecificationReport
...
const evaluateSpecification = (
  prepared: Candidate14PreparedData,
  plan: Candidate14Plan,
  familyStrategyHash: string,
  familyRunId: string,
  benchmarks: Candidate14BenchmarkResults,
): Result.Result<EvaluatedCandidate14Specification, Candidate14Failure> => {
  const strategyHash = canonicalHash('specification-strategy', {
    schemaVersion: 'bayn.candidate-14-specification-strategy.v1',
    familyStrategyHash,
  ...
  const runId = canonicalHash('specification-run', {
    schemaVersion: 'bayn.candidate-14-specification-run.v1',
    familyRunId,
  ...
  const stressed = replayCandidate14FixedOrderCosts(
    prepared.sessions,
    baselineWithTrace,
    plan.simulationStartIndex,
    plan.evaluationStartIndex,
  ...
  if (Result.isFailure(analysisResult)) {
    return Result.fail({ _tag: 'Candidate14QualificationFailure', cause: analysisResult.failure })
  }
  ...
  const report: Candidate14SpecificationReport = {
    specification: plan.specification,
...
export const evaluateCandidate14Development = (
  registration: Candidate14Registration,
  dataset: Candidate14Dataset,
  preflight: CandidateDevelopmentPreflightPass,
): Result.Result<Candidate14DevelopmentEvaluation, Candidate14Failure> => {
  if (preflight.protocolIdentity.protocolHash !== CANDIDATE_14_PROTOCOL_HASH) {
    return fail('preflight', `protocol hash ${preflight.protocolIdentity.protocolHash} differs from Candidate 14`)
  }
  const prepared = prepareCandidate14DevelopmentData(dataset)
  if (Result.isFailure(prepared)) return Result.fail(prepared.failure)
  const parameterHash = canonicalHash('parameters', {
    strategy: candidate14Protocol,
    simulation: candidate14SimulationProtocol,
    statistics: candidate14DevelopmentStatisticsPolicy,
    selectionMultiplicity: candidate14SelectionMultiplicity,
    priorAttemptIds: candidate14PriorAttemptIds,
  ...
  const behaviorHash = canonicalHash('behavior', candidate14BehaviorMaterial)
  if (Result.isFailure(behaviorHash)) return Result.fail(behaviorHash.failure)
  const terminalCash = candidate14TerminalLiquidationIsComplete()
  ...
  const familyStrategyHash = canonicalHash('family-strategy', {
    schemaVersion: 'bayn.candidate-14-family-strategy.v1',
  ...
  const familyRunId = canonicalHash('development-run', {
    schemaVersion: 'bayn.candidate-14-development-run.v1',
    ...
    developmentStart: CANDIDATE_14_DEVELOPMENT_START,
    developmentEnd: CANDIDATE_14_DEVELOPMENT_END,
  ...
  const plans = Result.all(
    candidate14Specifications.map((specification) =>
      buildCandidate14Plan(prepared.success.sessions, preflight, specification),
    ),
  )
  ...
  const benchmarkRunId = canonicalHash('benchmark-run', {
    schemaVersion: 'bayn.candidate-14-benchmark-run.v1',
  ...
  const benchmarks: Candidate14BenchmarkResults = {
  ...
  const reportMaterial = {
    schemaVersion: 'bayn.candidate-14-development-report.v1' as const,
    ...
    dataset: {
      snapshotId: dataset.snapshotId,
      firstSession: prepared.success.sessions.at(0)?.date ?? CANDIDATE_14_DEVELOPMENT_START,
      lastSession: prepared.success.sessions.at(-1)?.date ?? CANDIDATE_14_DEVELOPMENT_END,
    ...
    geometry: preflight,
    selection: {
      specificationCount: candidate14Specifications.length,
      familyMultiplicityDivisor: candidate14SelectionMultiplicity,
      priorAttemptCount: candidate14PriorAttemptIds.length,
    ...
    holdout: {
      start: CANDIDATE_14_HOLDOUT_START,
      end: CANDIDATE_14_HOLDOUT_END,
  ...
  const report: Candidate14DevelopmentReport = {
    ...reportMaterial,
